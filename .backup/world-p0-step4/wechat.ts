...
registerPromptTemplate({
  ...
  default: '你正在用微信和「我」聊天，你就是「{{name}}」。请像真人发微信一样回复：把想说的话拆成 1~{{maxBubbles}} 条简短、口语化的消息，'
    + '一条一句、自然断句，符合你的人设与说话风格；不要写成一大段长文，不要旁白、不要动作描写、不要括号心理活动。'
    + '严格只输出 JSON：{"messages":["第一条","第二条", ...]}，不要任何额外文字。',
});
registerPromptTemplate({
  ...
  default: '这是一个微信群聊。请让多位群成员像真人发微信一样接话：本轮可由 1~{{maxSpeakers}} 位成员发言，'
    + '每位成员可发 1~{{maxBubbles}} 条简短口语消息（一条一句，不要长文、不要旁白动作）。'
    + '发言要符合各自人设、彼此之间也能互相搭话、起哄、调侃。严格只输出 JSON：'
    + '{"replies":[{"speaker":"成员名","messages":["第一条","第二条"]}, ...]}，speaker 必须是给定成员之一，不要任何额外文字。',
});
registerPromptTemplate({
  id: 'wechat.moment_post', appId: 'wechat', appName: '微信', name: '朋友圈·发动态',
  desc: '让某位角色发一条朋友圈动态时的写法。',
  vars: [{ key: 'name', desc: '角色昵称' }],
  default: '你将扮演「{{name}}」发一条朋友圈动态。以第一人称、符合人设语气，60 字以内，像真实朋友圈口吻；'
    + '直接给正文，不要引号、不要解释、不要旁白。',
});
registerPromptTemplate({
  ...
  default: '下面是一条朋友圈动态，请让这些角色各自评论一句（符合各自人设，每句 30 字内，口语化）：\n{{roster}}\n\n'
    + '动态内容：{{moment}}\n\n严格只输出 JSON：{"comments":[{"speaker":"角色名","text":"评论"}, ...]}，speaker 必须是上面给定角色之一。',
});
...
async function onChatClick(t: HTMLElement): Promise<boolean> {
  ...
  if (row) { await handleMsgOp(chatId, t, row.getAttribute('data-wx-msg') || ''); return true; }
  return false;
}
