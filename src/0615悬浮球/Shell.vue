<template>
  ...
  >
    ...
    <!-- 状态栏宿主：原 0612 的 HTML 在 onMounted 后注入 -->
...
// ─── 常量 ───
// 注意：key 末尾 _v2 防止和 0612 悬浮球 共享 localStorage（避免两边互相覆盖状态）
...
async function mountStatusBar() {
  ...
  try {
    ...
    console.error('[0612悬浮球] 状态栏初始化失败:', e);
  }
}
...
.th-fab-panel {
  ...
  /* 注意：不使用 backdrop-filter / transform / filter / perspective，否则会成为面板内 position:fixed
     子元素的 containing block，导致原 0612 的弹窗 / 悬停提示 / 大图覆盖层定位错乱。 */
