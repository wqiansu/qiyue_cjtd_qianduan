// 储藏间 modal + 自定义 kind + add form。
// currentStashTab / isStashAllTab / _stashModalBusy 为本模块私有 whole-reassigned 状态；
//   getCurrentStashTab / getStatusStashAllTab getter 随之。
// currentFilterTag / currentlyCollapsed 为主文件 whole-reassigned 状态，本模块只写，通过 setCurrentFilterTag() / setCurrentlyCollapsed() setter 防引用断开。
// currentData 为主文件 whole-reassigned，本模块只读，通过 getCurrentStatusData() getter 懒读。
// showRecentOnly / showFavOnly 为对象引用型全局，仅属性读写，import 同一引用即可。
// ================================================================
...
// 储藏间 4 个固定 kind（主文件 openManagedEditModal 仍在用，故 export）。
export const STASH_FIXED_KINDS: ManagedKind[] = ['stash-item', 'stash-skill', 'stash-status', 'stash-clothing'];
// 未分类：固定 kind，作为删除自定义类别时卡片的归档目标；不参与初始数据/运行时导入/配发。
...
function openStashAddItemForm(kind: ManagedKind) {
  ...
  const fieldOrder: Record<string, string[]> = {
    ...
    'stash-clothing': ['名称','穿着部位','穿着情况','破损状态','外观详情','衣物状态','评价'],
  };
  const defaults: Record<string, Record<string, any>> = {
    'stash-item': {数量:1,简介:'',效果:'',评价:''},
    'stash-skill': {等级:1,简介:'',效果:'',评价:''},
    'stash-status': {效果:'',来源:'',持续时间:''},
    'stash-clothing': {穿着部位:'',穿着情况:'穿着',破损状态:'完好无缺',外观详情:'',衣物状态:'',评价:''},
  };
  const fields = isFixed ? (fieldOrder[kind] || ['名称','简介']) : ['名称','简介'];
  const defs = isFixed ? (defaults[kind] || {}) : {};
  const longFields = new Set(['简介','效果','评价','外观详情','衣物状态']);
  // 穿着情况/破损状态用下拉枚举（与 MVU schema 及 getDefaultEntry('clothing') 默认值口径一致）
  const enumFields: Record<string, Record<string, string[]>> = {
    'stash-clothing': {
      '穿着情况': ['穿着','脱下'],
      '破损状态': ['完好无缺','轻微破损','中度破损','严重破坏'],
    },
  };

  let h = `<div class="th-add-form">`;
  const enumsFor = enumFields[kind] || {};
  for (const f of fields) {
    ...
    if (enumsFor[f]) {
      const opts = enumsFor[f].map(o => `<option value="${escAttr(o)}" ${o === value ? 'selected' : ''}>${esc(o)}</option>`).join('');
      h += `<select class="th-edit-select th-add-field" data-add-field="${escAttr(f)}">${opts}</select>`;
    } else if (longFields.has(f)) h += `<textarea class="th-edit-textarea th-add-field" data-add-field="${escAttr(f)}" rows="3">${esc(value)}</textarea>`;
