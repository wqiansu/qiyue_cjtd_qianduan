...
export type InitialWriteResult = { written: number; skipped: number; book: string; created: boolean };
...
function ensureTagsForKind(realKind: string, tagNames: string[]): void {
  ...
  const bucket = allTags as Record<string, Record<string, { color: string; desc: string }>>;
  if (!bucket[realKind]) bucket[realKind] = {};
  let changed = false;
  for (const t of tagNames) {
    if (t && !bucket[realKind][t]) {
      bucket[realKind][t] = { color: TAG_COLOR_PALETTE[Math.floor(Math.random() * TAG_COLOR_PALETTE.length)], desc: '' };
      changed = true;
    }
  }
...
export async function readInitialDataFromWorldbook(): Promise<ParsedImport> {
  ...
  return parseInitialEntryContents(entryContents);
}

// 从「条目名→content」映射解析为 ParsedImport。
// 供绑定世界书路径与角色卡内嵌世界书（char-book）路径共用同一套解析逻辑。
export function parseInitialEntryContents(entryContents: Record<string, string>): ParsedImport {
  const byKind: Record<string, { name: string; item: ManagedItemV2 }[]> = {};
  const warnTags: string[] = [];
  const allTags = loadTags();
...
// 可选 opts.onPreview — 合并前先 diff「本地已有 vs 待读入」，调 onPreview 弹差异列表，
// 玩家返回 true 才继续合并。默认无 onPreview 时保持原行为。
export async function mergeInitialDataIntoLocal(parsed: ParsedImport, opts?: { onPreview?: (willAdd: { kind: string; name: string }[]) => Promise<boolean> }): Promise<{ added: number; skipped: number; kinds: string[] }> {
  let added = 0, skipped = 0;
  const touchedKinds: string[] = [];
  // 先 diff 出「将新增」清单供预览
  const willAdd: { kind: string; name: string }[] = [];
  if (opts?.onPreview) {
    for (const [kindKey, items] of Object.entries(parsed.byKind)) {
      ...
      for (const { name } of items) {
        if (!current[name]) willAdd.push({ kind, name });
      }
    }
    const ok = await opts.onPreview(willAdd);
    if (!ok) return { added: 0, skipped: 0, kinds: [] };
  }
  for (const [kindKey, items] of Object.entries(parsed.byKind)) {
    const kind = kindKey as ManagedKind;
...
// 改为 export，供 links-init 复用（蓝灯 constant + 禁用，数据源条目统一构造）
...
// 读取前差异预览 modal — 列「将新增」卡片清单，确认(返回 true)/取消(返回 false)。
// 供 init-manager 看板「读取」按钮作为 mergeInitialDataIntoLocal 的 onPreview 使用。
export function openDiffPreviewModal(willAdd: { kind: string; name: string }[]): Promise<boolean> {
  return new Promise((resolve) => {
    if (!willAdd.length) { toastr?.info?.('没有需要新增的卡片（本地已全部存在）'); resolve(false); return; }
    // 按kind分组
    const byKind: Record<string, string[]> = {};
    for (const { kind, name } of willAdd) (byKind[kind] ||= []).push(name);
    const rows: string[] = [];
    for (const [kind, names] of Object.entries(byKind)) {
      const label = (() => { try { return getStashKindCfg(kind as ManagedKind).label; } catch { return kind; } })();
      rows.push(`<div class="th-init-diff-group"><div class="th-init-diff-kind">${esc(label)} <span class="th-init-diff-cnt">${names.length}</span></div><div class="th-init-diff-names">${names.map(n=>`<span class="th-init-diff-name">${esc(n)}</span>`).join('')}</div></div>`);
    }
    const h=`<div class="th-init-diff" style="padding:14px">
      <div style="color:var(--tx2);line-height:1.7;font-size:13px;margin-bottom:10px">将从初始数据增量补入 <b>${willAdd.length}</b> 张卡片（已有同名不覆盖）：</div>
      <div class="th-init-diff-list" style="max-height:300px;overflow-y:auto;display:grid;gap:10px;margin-bottom:16px">${rows.join('')}</div>
      <div style="display:flex;gap:8px;justify-content:flex-end">
        <button class="th-btn" id="th-diff-cancel">取消</button>
        <button class="th-btn th-btn-primary" id="th-diff-confirm"><i class="fa-solid fa-check"></i> 确认补入</button>
      </div>
    </div>`;
    openModal2('读取前差异预览', h);
    setTimeout(() => {
      qs('#th-diff-cancel')?.addEventListener('click', () => { closeModal2(); resolve(false); });
      qs('#th-diff-confirm')?.addEventListener('click', () => { closeModal2(); resolve(true); });
    }, 40);
  });
}
...
export async function writeInitialItemsForKind(kind:ManagedKind, items:InitialWriteItem[], mode:InitialWriteMode): Promise<InitialWriteResult> {
...
// 按 kind「覆盖」写入世界书 [初始·xxx]（替换该 kind 的全部内容，而非 dedupe/append）。
// - location/event/dlc：整条目 content 替换为该 kind 的 JSON 数组。
// - stash-*：只替换 {groups} 里该 kind 的那一组，其余储藏间子类原样保留（互不影响）。
// 供「初始卡片层 → 世界书」单类写出复用。items 为空时写空数组/空组（= 清空该类世界书初始数据）。
export async function setInitialItemsForKind(kind: ManagedKind, items: InitialWriteItem[]): Promise<{ written: number; book: string; created: boolean }> {
  const targetName = getInitialEntryNameForKind(kind);
  if (!targetName) return { written: 0, book: '', created: false };
  const found = await findInitialEntry(targetName);
  let content = '';
  if (targetName === INITIAL_ENTRY_NAMES.stash) {
    const groups = readInitialGroups(found.content);
    groups[kind] = items.slice();
    content = JSON.stringify({ format: 'th-managed-multi-v1', groups }, null, 2);
  } else {
    content = JSON.stringify(items.slice(), null, 2);
  }
  const result = await upsertInitialContent(targetName, content, found.book);
  return { written: items.length, book: result.book, created: !found.exists || result.created };
}

// 把 ManagedItemV2 字典转成初始写入项数组（供 init-cards 层 → 世界书）。导出供 init-manager 复用。
export function managedMapToInitialItems(map: Record<string, ManagedItemV2>): InitialWriteItem[] {
  return Object.entries(map).map(([name, item]) => toInitialWriteItem(name, item));
}
...
export async function importStashKind(kind: ManagedKind, text: string, applyTag?: string) {
  ...
  // 同名卡片用一次确认统一处理（确定=全部覆盖，取消=全部跳过），避免逐条弹窗
...
// 汇报导入结果（统一 toast）
// skipped：因同名已存在被跳过的条数（imported=0 且 skipped>0 时不再误报"未识别"）
...
// 校验类型：location/event/dlc 为单 kind JSON 数组；stash 为多 kind {format,groups}；links 为关联图。
export type InitPayloadType = 'location' | 'event' | 'dlc' | 'stash' | 'links';
const INIT_ITEM_SCHEMA = z.object({
  name: z.string().min(1),
  desc: z.string(),
  tags: z.array(z.string()).optional(),
  inject: z.string().optional(),
  links: z.object({ locations: z.array(z.string()).optional(), events: z.array(z.string()).optional(), dlcs: z.array(z.string()).optional() }).optional(),
}).passthrough();
const INIT_MULTI_SCHEMA = z.object({ format: z.string().optional(), groups: z.record(z.string(), z.array(INIT_ITEM_SCHEMA)) });
const INIT_LINKS_SCHEMA = z.object({ format: z.string().optional(), links: z.record(z.string(), z.any()) });

// validateInitPayload: 校验初始数据文本是否合法。
// - JSON 路径：jsonrepair 容错修复 → JSON.parse → zod schema 校验（按 type）
// - YAML 路径：yaml.parse → 同 zod 校验
// 返回 {ok, errors, repairedText?}。ok=false 时 errors 含中文错误定位；repairedText 为 jsonrepair 修复后的文本（供看板展示修复结果）。
export function validateInitPayload(text: string, type: InitPayloadType): { ok: boolean; errors: string[]; repairedText?: string } {
  const trimmed = (text || '').trim();
  if (!trimmed) return { ok: false, errors: ['内容为空'] };
  const errors: string[] = [];
  let parsed: any = null;
  let repairedText: string | undefined;
  // 优先按 JSON（jsonrepair 容错）
  const looksJson = trimmed.startsWith('{') || trimmed.startsWith('[') || trimmed.startsWith('th-');
  if (looksJson || !/^[#\-\s]/.test(trimmed)) {
    try {
      repairedText = jsonrepair(trimmed);
      parsed = JSON.parse(repairedText);
    } catch (e1) {
      try {
        parsed = yamlParse(trimmed); // 非法 JSON 可能是 YAML
        repairedText = trimmed;
      } catch (e2) {
        errors.push('JSON 解析失败：' + String((e1 as Error).message || e1).slice(0, 200));
        return { ok: false, errors };
      }
    }
  } else {
    try {
      parsed = yamlParse(trimmed);
      repairedText = trimmed;
    } catch (e) {
      errors.push('YAML 解析失败：' + String((e as Error).message || e).slice(0, 200));
      return { ok: false, errors };
    }
  }
  // schema 校验
  try {
    if (type === 'links') {
      INIT_LINKS_SCHEMA.parse(parsed);
    } else if (type === 'stash') {
      INIT_MULTI_SCHEMA.parse(parsed);
    } else {
      // 单 kind：数组
      if (!Array.isArray(parsed)) { errors.push('期望 JSON 数组（单类别数据应为 `[{name,desc,...}]`）'); return { ok: false, errors }; }
      z.array(INIT_ITEM_SCHEMA).parse(parsed);
    }
  } catch (e) {
    if (e instanceof z.ZodError) {
      for (const iss of (e as any).issues || (e as any).errors || []) {
        const path = iss.path && iss.path.length ? '（路径 ' + iss.path.join('.') + '）' : '';
        errors.push(iss.message + path);
      }
      if (!errors.length) errors.push('结构校验失败');
    } else {
      errors.push('结构校验失败：' + String((e as Error).message || e).slice(0, 200));
    }
    return { ok: false, errors };
  }
  return { ok: true, errors: [], repairedText };
}

// 从 payload 类型推断要导入的 kind（供导入闸门复用 parseManagedImport 之前快速校验）
function inferStashKindsFromPayload(text: string): InitPayloadType {
  const t = (text || '').trim();
  if (t.includes('"format":"th-links-graph-v1"') || t.includes("'format':'th-links-graph-v1'")) return 'links';
  if (t.startsWith('{') || t.includes('"groups"') || t.includes("'groups'")) return 'stash';
  // 数组则需调用方定 kind，这里默认 stash 物品兜底
  return 'stash';
}

// 导入前置闸门：在 importStashKind/importAllStashKinds 调用前校验，坏数据 toast 拦截并 return false。
// 成功返回 true。不接管后续同名覆盖 confirm 逻辑。
function assertImportPayload(text: string, label: string): boolean {
  const type = inferStashKindsFromPayload(text);
  const v = validateInitPayload(text, type === 'stash' ? 'stash' : 'links');
  if (!v.ok) {
    toastr?.error?.(`导入失败：${label}数据格式不合法\n${v.errors.slice(0, 3).join('；')}`);
    console.warn('[初始化校验闸门] 拦截坏数据', label, v.errors);
    return false;
  }
  return true;
}