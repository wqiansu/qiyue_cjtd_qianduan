...
// 批次6：从「条目名→content」映射解析为 ParsedImport（从 readInitialDataFromWorldbook 抽出，零行为变化）。
...
// 批次2 #13：加可选 opts.onPreview — 合并前先 diff「本地已有 vs 待读入」，调 onPreview 弹差异列表，
// 玩家返回 true 才继续合并。默认无 onPreview 时保持原行为（现有调用点不传，零变化）。
...
// 批次2 #13：读取前差异预览 modal — 列「将新增」卡片清单，确认(返回 true)/取消(返回 false)。
...
// 批次7 反馈5：按 kind「覆盖」写入世界书 [初始·xxx]（替换该 kind 的全部内容，而非 dedupe/append）。
...
export function importStashKind(kind: ManagedKind, text: string, applyTag?: string) {
  // 批次2 #12：导入前置校验闸门（坏数据拦截，不接管后续同名覆盖逻辑）
...
// 成功返回 true。不接管后续同名覆盖 confirm 逻辑（反馈2 保留）。
