// 可视化编辑器（init-visual-editor）。
// 在 init-manager 板 5 个条目行各加「可视化编辑」按钮，点击打开 modal，按卡片网格可视化增删改，
// 确认后重新序列化写回世界书条目 content —— 不触碰 localStorage managed 卡片，不污染运行时数据，
// 格式由严格序列化保证合法（用户不会因手写 JSON 出错）。
//   ① 储藏间卡片无 links 关联字段（储藏间 bindsWorldbook:false、与关联图无关）。
//   ② 储藏间4类 desc 是结构化 JSON（见 stash-modal addManagedItem / getDefaultEntry），
//      可视化按 kind 把变量字段拆成独立输入框（物品/技能/状态/衣物），未分类走简单 name+flat 文本 desc。
//   ③ 占位字灰白 + 每卡片「复制」按钮（插入同值副本）。
//   location/event/dlc 卡片保留 links（这三类有关联），desc 是纯文本不拆变量。
//   links kind 卡片是 node 网格（name + 3 CSV）。
// 命令式 innerHTML + openModal2 + data 委托，不引 Vue。
// 向下兼容：不改条目名、不改 localStorage key、不破坏现有函数签名；本编辑器是「编辑」按钮的可视化补充，原 wb-inspector 文本编辑入口保留。
// ================================================================
import {
  INITIAL_ENTRY_NAMES,
  type ManagedKind,
...
// ==================== 初始卡片层 · 可视化编辑器 ====================
// 复用上方可视化编辑器的卡片网格 + 结构化字段拆分（物品/技能/状态/衣物各字段独立输入框，
// 地点/事件/DLC 走 desc + links），但数据源改为「初始卡片层」内存 map（不碰世界书）。
// 单 kind 一屏，增删改/复制卡片后点「保存」整组覆盖写回初始层。
type InitVKind = 'location' | 'event' | 'dlc' | 'stash-item' | 'stash-skill' | 'stash-status' | 'stash-clothing';
const INITV_LABEL: Record<InitVKind, string> = {
  location: '地点', event: '事件', dlc: 'DLC',
  'stash-item': '物品', 'stash-skill': '技能', 'stash-status': '状态', 'stash-clothing': '衣物',
};

// ManagedItemV2 map → VCard[]（单 kind；toVCard 会按 desc 是否为 JSON 自动判结构化/纯文本）
function initMapToVCards(map: Record<string, ManagedItemV2>): VCard[] {
  return Object.entries(map).map(([name, it]) => {
    const raw = { name, desc: it.desc, tags: it.tags, inject: it.inject, links: it.links };
    return toVCard(raw);
  }).filter(Boolean) as VCard[];
}
// VCard → ManagedItemV2（保留原 order/favorite/locked 等不可见字段）
function vCardToInit(c: VCard, prev?: ManagedItemV2): ManagedItemV2 {
  const desc = (c.fields && Object.keys(c.fields).length) ? JSON.stringify(c.fields) : (c.descFlat ?? '');
  const out: ManagedItemV2 = { ...(prev || { desc: '', tags: [] }), desc, tags: c.tags ?? [] };
  if (c.inject) out.inject = c.inject; else delete out.inject;
  if (c.links && (c.links.locations?.length || c.links.events?.length || c.links.dlcs?.length)) out.links = c.links;
  else delete out.links;
  return out;
}

// grp 标记用 kind 本身（renderVCard/emptyCardFor 已支持 stash-* 与 location/event/dlc；
// 这里 location/event/dlc 不走 '__default__' 而直接用 kind，需让 renderVCard 走 desc+links 分支——
// 见下方 renderInitVCard 适配）。
function renderInitVCard(c: VCard, kind: InitVKind): string {
  // 结构化四类直接复用
  if (isStructuredKind(kind)) return renderVCard(c, kind, 0);
  // location/event/dlc：复用 __default__ 分支（desc + links）
  return renderVCard(c, '__default__', 0);
}

export function openInitLayerVisualEditor(
  kind: InitVKind,
  getMap: () => Record<string, ManagedItemV2>,
  saveMap: (map: Record<string, ManagedItemV2>) => void,
  onClose?: () => void,
): void {
  const prevMap = getMap();
  const cards = initMapToVCards(prevMap);
  const grp = isStructuredKind(kind) ? kind : '__default__';
  const cardsHtml = cards.map(c => renderInitVCard(c, kind)).join('');
  const title = `<i class="fa-solid fa-pen-ruler"></i> 可视化编辑初始卡片 · ${INITV_LABEL[kind]}`;
  const body = `<div class="th-init-ved">
    <div class="th-init-ved-tip">编辑「初始卡片层（基线）」——不影响实时卡片与世界书。增删改/复制后点「保存」整组覆盖写回初始层。</div>
    <div class="th-init-ved-grps">
      <div class="th-init-ved-grp" data-ved-grp="${escAttr(grp)}">
        <div class="th-init-ved-cards">${cardsHtml}</div>
        <button class="th-btn-sm th-init-ved-add" data-ved-grp="${escAttr(grp)}" type="button"><i class="fa-solid fa-plus"></i> 新增卡片</button>
      </div>
    </div>
    <div class="th-init-ved-footer">
      <button class="th-btn" id="th-ived-cancel" type="button"><i class="fa-solid fa-xmark"></i> 取消</button>
      <button class="th-btn th-btn-primary" id="th-ived-save" type="button"><i class="fa-solid fa-floppy-disk"></i> 保存到初始层</button>
    </div>
  </div>`;
  openModal2(title, body, { maxWidth: 'min(920px,94vw)', reset: true });
  setTimeout(() => {
    // 复用卡片复制/删除绑定
    qsa('.th-init-ved-card').forEach(cardEl => bindCardActs(cardEl as HTMLElement));
    // 新增卡片
    qsa('.th-init-ved-add[data-ved-grp]').forEach(btn => {
      btn.addEventListener('click', () => {
        const g = btn.getAttribute('data-ved-grp') || '__default__';
        const cardsWrap = btn.closest('.th-init-ved-grp')?.querySelector('.th-init-ved-cards');
        if (!cardsWrap) return;
        const div = __doc.createElement('div');
        div.innerHTML = renderInitVCard(emptyCardFor(g), kind);
        const cardEl = div.firstElementChild as HTMLElement;
        if (cardEl) { cardsWrap.appendChild(cardEl); bindCardActs(cardEl); }
      });
    });
    qs('#th-ived-cancel')?.addEventListener('click', () => { closeModal2(); onClose?.(); });
    qs('#th-ived-save')?.addEventListener('click', () => {
      // 回读该组所有卡片
      const grpEl = qs(`.th-init-ved-grp[data-ved-grp="${grp}"]`);
      if (!grpEl) { closeModal2(); return; }
      const next: Record<string, ManagedItemV2> = {};
      let blank = false; let order = 0;
      (grpEl.querySelectorAll('.th-init-ved-card') as NodeListOf<HTMLElement>).forEach(el => {
        const name = (el.querySelector<HTMLInputElement>('.th-init-ved-name')?.value || '').trim();
        if (!name) { blank = true; return; }
        const tags = parseCsv(el.querySelector<HTMLInputElement>('.th-init-ved-tags')?.value || '');
        const inject = (el.querySelector<HTMLInputElement>('.th-init-ved-inject')?.value || '').trim() || undefined;
        const card: VCard = { name, tags, inject };
        if (isStructuredKind(kind)) {
          const spec = STASH_FIELD_SPEC[kind as ManagedKind]!;
          const fields: Record<string, string | number> = {};
          el.querySelectorAll('[data-vf-field]').forEach(inp => {
            const f = inp.getAttribute('data-vf-field') || '';
            const v = (inp as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement).value;
        ...
        next[name] = vCardToInit(card, prevMap[name]);
        next[name].order = order++;
      });
      if (blank) { toastr?.warning?.('存在空名称卡片，已忽略未命名项'); }
      saveMap(next);
      toastr?.success?.(`已保存 ${Object.keys(next).length} 张初始卡片`);
      closeModal2();
      onClose?.();
    });
  }, 50);
