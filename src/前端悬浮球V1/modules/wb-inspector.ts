...
// 导出供 init-manager 面板「编辑」按钮复用（按 worldbook+uid 打开同款条目编辑器）
export async function openWorldbookEntryDetail(worldbookName:string, uid:number) {
...
// 布尔字段用 galgame 滑动开关(隐藏 checkbox + slider)
...
function getWbFieldValue(field:string): string {
  ...
  if(!el) return '';
  // 滑动开关是 checkbox,读 checked 转 'true'/'false'(兼容 boolFromSelect)
...
function bindWorldbookEntryEditor(worldbookName:string, uid:number) {
  ...
  qs('#th-wb-entry-save')?.addEventListener('click',async()=>{
    try{
      ...
      // 保存后局部更新 — 不重开识别器,只更新本行 chip + 恢复识别器滚动位置
...
// 保存后局部刷新识别器该行(不重开 modal,恢复滚动位置)
// 全量重渲染列表但保持搜索过滤 + 滚动位置(比单行刷更稳,数据一致)
