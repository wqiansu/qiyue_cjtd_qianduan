// 物品/技能网格 + 背包/技能弹窗（解耦阶段 1h）。
// 从 status-bar-init.ts 纯移动；行为保持一致。
// currentData/isEditMode 通过主文件 getter 懒读，避免 whole-reassigned 状态引用断开。
// ================================================================
import { esc, escAttr, qs, qsa, editableInput, editableTextarea } from '../lib/dom-utils';
import { safeTriggerSlash } from '../lib/tavern-api';
import { collectStatDataChange } from '../lib/variable-review';
import {
  getUser,
  getUN,
  getUserKey,
...
// --- User 背包 ---
export function openUserBag(data:Record<string,any>) {
...
// --- NPC 背包 ---
// §10.11 ⑤fix:改 openModal2 叠加,关闭后回 NPC 详情而非回状态栏
export function openNPCBag(npcName:string) {
  ...
  openModal2(`<i class="fa-solid fa-box-open"></i> `+esc(npcName)+' · 背包',h);
  bindGridItemClicks(npcName, entries, basePath, 'npc');
}

// --- User 技能 ---
export function openUserSkill(data:Record<string,any>) {
  const u=getUser(data); const uname=getUN(data); const ukey=getUserKey(data);
...
// --- NPC 技能 ---
// §10.11 ⑤fix:改 openModal2 叠加,关闭后回 NPC 详情而非回状态栏
export function openNPCSkill(npcName:string) {
  const currentData = getCurrentStatusData();
  const info=getNPCInfo(currentData,npcName); if(!info) return;
  ...
  openModal2(`<i class="fa-solid fa-scroll"></i> `+esc(npcName)+' · 技能',h);
...
function showItemDetail(name:string, it:any, editPath:string, ownerName:string, ownerType:string) {
  ...
  qs('.th-btn-use-item')?.addEventListener('click',function(this:HTMLElement){
    ...
    try{safeTriggerSlash('/setinput '+tavernMacro('input')+' <'+on+'使用了'+it+'>')}catch(e){ void e; }
    // §10.11 ⑤fix:NPC 背包是叠加层(modal2),关 modal2 回 NPC 详情;user 背包是主层(modal1),关 modal1
    if(ownerType==='npc') closeModal2(); else closeModal();
  });
  // 丢弃按钮
...
function showSkillDetail(name:string, sk:any, editPath:string, ownerName:string, ownerType:string) {
  const detailEl=qs('#th-grid-detail'); if(!detailEl) return;
  const gridEl=detailEl.previousElementSibling as HTMLElement;
  if(gridEl) gridEl.style.display='none';
  let h='';
  ...
  qs('.th-btn-use-skill')?.addEventListener('click',function(this:HTMLElement){
    ...
    try{safeTriggerSlash('/setinput '+tavernMacro('input')+' <'+on+'使用了'+sk+'>')}catch(e){ void e; }
    // §10.11 ⑤fix:NPC 技能是叠加层(modal2),关 modal2 回 NPC 详情;user 技能是主层(modal1)
    if(ownerType==='npc') closeModal2(); else closeModal();
  });
  qs('.th-btn-back-grid')?.addEventListener('click',()=>{
    detailEl.style.display='none'; detailEl.innerHTML='';
