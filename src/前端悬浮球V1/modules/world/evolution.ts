...
import {
  EvoEntry, EvoVarChange, EvoWbRef, EvoActor,
...
type SheetState =
  ...
  | { kind: 'charConfig'; actorId: string }           // #3 角色配置（专属世界书/额外设定/人设/内置提示词）
  | { kind: 'wbPick'; actorId?: string; target: 'actor' | 'global' } // #3/#5 给角色或全局选世界书条目
  | { kind: 'settings' }                              // #3 世界演化设置（API/正文/注入/全局世界书）
...
function listHtml(): string {
  ...
  const head = `<div class="th-evo-head">
    ...
    <span class="th-evo-head-ops">
      <button class="th-evo-chip" data-evo-pulse type="button" title="世界节拍总览">${iconHtml('fa-wave-square')} 节拍</button>
      ...
      <button class="th-evo-primary" data-evo-pick type="button">${iconHtml('fa-user-plus')} 添加角色</button>
    </span>
  </div>`;
  // #3 世界背景演化线：三套内置预设固定在主 modal 上，一点即开线
  const existPresetKeys = new Set(actors.map(a => a.presetKey).filter(Boolean) as string[]);
  const presetChips = WORLD_PRESETS.map(p =>
    `<button class="th-evo-wpreset${existPresetKeys.has(p.key) ? ' th-evo-wpreset-on' : ''}" data-evo-preset="${esc(p.key)}" type="button" title="${esc(p.group)}">
      ${iconHtml('fa-earth-asia')} ${esc(p.name)}${existPresetKeys.has(p.key) ? ' ·已开' : ''}</button>`).join('');
  const worldRow = `<div class="th-evo-wbar">
    <span class="th-evo-wbar-label">${iconHtml('fa-globe')} 世界背景演化线</span>
    <div class="th-evo-wbar-chips">${presetChips}
      <button class="th-evo-wpreset th-evo-wpreset-custom" data-evo-pick-world="" type="button">${iconHtml('fa-plus')} 自定义维度</button>
    </div>
  </div>`;

  // #1 联合推演固定在顶部工具条（不再选中后弹条）
  const selN = actors.filter(a => _selected.has(a.id)).length;
  const toolbar = actors.length ? `<div class="th-evo-toolbar">
    <span class="th-evo-tb-info">${selN > 0 ? `已选 ${selN} 个` : '勾选多个对象可联合推演'}</span>
    <span class="th-evo-tb-ops">
      ${selN > 0 ? `<button class="th-evo-chip th-evo-chip-sm" data-evo-selclear type="button">取消</button>` : ''}
      <button class="th-evo-primary th-evo-tb-co" data-evo-coadvance type="button" ${(_busy || selN < 2) ? 'disabled' : ''}>${iconHtml('fa-people-group')} 联合推演${selN >= 2 ? `（${selN}人·一次）` : ''}</button>
    </span>
  </div>` : '';

  if (!actors.length) {
    return `<div class="th-evo-app" data-evo-view="list">${head}${worldRow}
      <div class="th-evo-empty">${iconHtml('fa-seedling')}
        <div>还没有演化对象</div>
        <div class="th-evo-empty-sub">点上方「世界背景演化线」开一条世界线，或「添加角色」从离场 NPC / 联系人里挑一个，让你不在场时世界也在悄悄转动。</div>
      </div></div>`;
  }
  const cards = actors.map(a => actorCardHtml(a)).join('');
  return `<div class="th-evo-app" data-evo-view="list">${head}${worldRow}${toolbar}
    <div class="th-evo-card-grid">${cards}</div></div>`;
}

// 单个演化对象卡片（紧凑网格；推进/配置按钮直接在卡上，不必进详情）
function actorCardHtml(a: EvoActor): string {
  const last = a.timeline[a.timeline.length - 1];
  const preview = last ? esc(last.summary.slice(0, 60)) : '尚未推进演化';
  ...
  const promptFlag = a.useCustomPrompt ? `<span class="th-evo-card-flag" title="使用内置专属提示词">${iconHtml('fa-wand-magic-sparkles')}</span>` : '';
  const injectFlag = a.injectEnabled ? `<span class="th-evo-card-flag" title="演化注入正文已开">${iconHtml('fa-syringe')}</span>` : '';
  return `<div class="th-evo-card${_selected.has(a.id) ? ' th-evo-card-sel' : ''}${isWorld ? ' th-evo-card-world' : ''}" data-evo-card="${esc(a.id)}">
    <label class="th-evo-card-ck" title="选中以联合推演"><input type="checkbox" data-evo-sel="${esc(a.id)}" ${checked}></label>
    <button class="th-evo-card-open" data-evo-open="${esc(a.id)}" type="button">
      ${isWorld ? `<span class="th-evo-av th-evo-av-world">${iconHtml('fa-earth-asia')}</span>` : avatarChip(a.name)}
      <span class="th-evo-card-body">
        <span class="th-evo-card-name">${esc(a.name)} ${tag} ${promptFlag}${injectFlag}</span>
        <span class="th-evo-card-meta"><span class="th-evo-actor-count">${a.timeline.length} 段</span> ${last ? timeLabel(last.ts) : ''}</span>
        <span class="th-evo-card-preview">${preview}</span>
      </span>
    </button>
    <div class="th-evo-card-ops">
      <button class="th-evo-card-advance" data-evo-card-advance="${esc(a.id)}" type="button" ${_busy ? 'disabled' : ''}>${iconHtml('fa-forward')} 推进</button>
      <button class="th-evo-card-cfg" data-evo-card-cfg="${esc(a.id)}" type="button" title="配置">${iconHtml('fa-id-card')}</button>
    </div>
  </div>`;
}
...
function settingsInnerHtml(): string {
  ...
  return `<div class="th-evo-form">
    ...
    <div class="th-evo-set-glabel">${iconHtml('fa-book-bookmark')} 默认绑定世界书条目（全局）</div>
    <div class="th-evo-set-hint">这里绑定的条目会作为「世界观锚点」附加进每一次推演（单人/联合/世界背景都带），让所有演化共享同一套设定基底。</div>
    <div class="th-evo-wbrefs">${
      (_gWb && _gWb.length)
        ? _gWb.map((r, i) => `<span class="th-evo-wbref">${iconHtml('fa-book')} ${esc(r.name)}<button class="th-evo-wbref-x" data-evo-gwb-del="${i}" type="button">${iconHtml('fa-xmark')}</button></span>`).join('')
        : `<span class="th-evo-empty-sub">未绑定（推演只用各对象自己的设定）</span>`
    }</div>
...
// ==================== 世界节拍总览 sheet（跨对象最近演化一览）====================
function pulseInnerHtml(): string {
  const actors = getActors();
  // 收集所有时间线条目，按时间倒序取最近若干条，标注归属
  ...
  const stat = `<div class="th-evo-pulse-stat">
    ...
    <span>${iconHtml('fa-clock-rotate-left')} 总演化 ${rows.length} 段</span>
  </div>`;
  if (!recent.length) return `<div class="th-evo-form">${stat}<div class="th-evo-empty-sub" style="padding:14px">还没有任何演化。先推进几次，这里会汇总你不在场时世界各处的动静。</div></div>`;
  const list = recent.map(r => `<div class="th-evo-pulse-row${r.isWorld ? ' th-evo-pulse-world' : ''}">
    <span class="th-evo-pulse-tag">${r.isWorld ? iconHtml('fa-earth-asia') : iconHtml('fa-user')} ${esc(r.name)}</span>
    <span class="th-evo-pulse-when">${r.e.worldTime ? esc(r.e.worldTime) : timeLabel(r.e.ts)}</span>
    <span class="th-evo-pulse-sum">${esc(r.e.summary.slice(0, 70))}</span>
  </div>`).join('');
  return `<div class="th-evo-form">${stat}
    <div class="th-evo-set-hint">把所有对象最近的演化按时间汇到一起，一眼看清「你不在场时，世界整体在往哪走」。</div>
    <div class="th-evo-pulse-list">${list}</div>
  </div>`;
}
...
function sheetHtml(): string {
  ...
  } else if (_sheet.kind === 'pulse') {
    title = '世界节拍总览';
    inner = pulseInnerHtml();
...
async function onClick(e: Event): Promise<void> {
  ...
  if (t.closest('[data-evo-pulse]')) { openSheet({ kind: 'pulse' }); return; }
  if (t.closest('[data-evo-settings]')) { _gWb = (getEvoConfig().globalWbRefs || []).slice(); openSheet({ kind: 'settings' }); return; }
  ...
  if (_view.name === 'list') {
    ...
    if (wbtn) {
      const v = ask('自定义世界演化维度（如：宫廷权斗 / 灵气复苏）：');
...
async function onSheetClick(t: HTMLElement): Promise<boolean> {
  ...
  if (_sheet.kind === 'charConfig') {
    const aid = _sheet.actorId;
    if (t.closest('[data-evo-wb-add]')) { _wbBook = ''; _wbEntries = []; openSheet({ kind: 'wbPick', actorId: aid, target: 'actor' }); return true; }
  ...
  if (_sheet.kind === 'wbPick') {
    ...
    if (ent) {
      ...
      if (_sheet.target === 'global') {
        if (e && !_gWb.some(r => r.book === _wbBook && r.uid === uid)) _gWb.push({ book: _wbBook, uid, name: e.name });
        openSheet({ kind: 'settings' });
      } else {
        if (e && !_ceWb.some(r => r.book === _wbBook && r.uid === uid)) _ceWb.push({ book: _wbBook, uid, name: e.name });
        openSheet({ kind: 'charConfig', actorId: _sheet.actorId || '' });
      }
      return true;
    }
    return true;
  ...
  if (_sheet.kind === 'settings') {
    if (t.closest('[data-evo-gwb-add]')) { _wbBook = ''; _wbEntries = []; openSheet({ kind: 'wbPick', target: 'global' }); return true; }
...
async function runAdvance(actorId: string, opts: { span: string; worldTime: string; direction: string }): Promise<void> {
  ...
  try {
    ...
    // #4 提示词解析：勾选了「专属内置提示词」用 customPrompt，否则用默认模板（world 用 evolution.world，角色用 evolution.advance）
...
// #5 拼全局绑定世界书条目的内容（所有推演共享的世界观锚点）。异步取内容，失败降级空。
...
function openApp(): void {
  openModal2(`${iconHtml('fa-seedling')} 世界演化`, `<div class="th-evo th-phone"><div class="th-phone-island"><span class="th-phone-cam"></span></div><div id="${RID}" class="th-phone-screen"></div></div>`, {
