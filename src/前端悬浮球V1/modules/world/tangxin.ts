...
// 直播向社交 APP：世界观角色开直播间，玩家进场互动（弹幕/送礼/点单/连麦/福袋，主播 AI 回应 + 围观弹幕）。
// 复刻真实 PC 直播站外观：左窄图标轨(推荐/直播/关注/榜单/回放/我/设置) + 主区(顶栏/分类/瀑布流) + 右侧栏；
//   直播间走「左画面 + 右弹幕」铁律布局。
// 好感5阶解锁/打赏点单/连麦PK/福袋/粉丝团/主播主页/榜单/回放/下播结算/钱包账单。
// 设置：上下文/API利用(含按钮→产出对照图)/世界书注入/提示词(含破限)/记忆同步/主动推送/
//   互动风格尺度/隐私/钱包/画面出图/外观/楼层自动触发/数据管理。
// 架构：单 modal + revive 重建 + _stage/_sheet 状态机 + 事件委托。图片一律外框+文字描述(文生图未接)。
