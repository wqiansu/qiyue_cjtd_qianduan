...
// 三栏 master-detail（.thw-bili-*）：左导航(分区/动态/收藏/历史/我的/设置 + 投稿) /
//   中内容(视频瀑布流 / 播放页) / 右检视(排行榜 ↔ 相关推荐 + UP名片)。
// 真实感：仿 B站网页版——分区栏、瀑布流封面卡、播放页弹幕飞过 + 三连(赞/币/藏) + 关注 +
//   评论楼中楼、UP名片、排行榜、相关推荐、玩家投稿、二创鬼畜衍生链、催更→UP回应、指定角色开号、转发到微信。
// 设置：上下文与世界书(内联条目复选·桶外) / API利用 / 功能提示词(破限置顶) /
//   分区与播放 / 自动触发 / 记忆与数据(合并)。提示词信息密度高，玩家可自定义。
