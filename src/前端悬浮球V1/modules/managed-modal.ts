// location/event/dlc 共用 managed modal 总览 + 编辑 + 关联。
//
// 可变全局归属：
// - 本模块私有：currentSortMode / SORT_PREFS_KEY / 类型 SortMode / SearchMode。
// - 主文件跨模块共享（whole-reassigned）：currentFilterTag（getCurrentFilterTag/setCurrentFilterTag）、
//   currentlyCollapsed（getCurrentlyCollapsed/setCurrentlyCollapsed）——本模块经 getter 读、setter 写，防引用断开。
// - 主文件对象引用型（仅属性读写）：showRecentOnly / showFavOnly / batchMode / batchSelection ——直接 import 同一引用。
//
// 循环依赖：本模块从 '../status-bar-init' import 主文件私有函数（tavernMacro/bindManagedCardEvents 等，
// 函数体内延迟调用，ESM live binding 安全）。
// ================================================================
...
// 循环依赖（ESM live binding + 函数体内延迟调用）：
...
import { openRefineModal, openRewriteModal } from './ai-refine';
import { openExportToWorldbookModal } from './card-export';
import { openTagManagerModal, renderTagFilterBar } from './tag-manager';
import {
  downloadText,
...
export function renderManagedBuckets(kind: ManagedKind, _idPrefix: string, query: string = ''): string {
  ...
  // 最近10张模式：跨桶平铺显示最近编辑的10张（先过滤再按时间取前10）
  ...
  for (const [name, item] of filteredEntries) {
    ...
    if (!item.tags || item.tags.length === 0) {
      ...
      // 卡片带的标签全部「未建立」时，会落入无桶 → 计数有、却不显示。兜底归入未分类桶，保证可见。
      if (!placed) buckets[''].push([name, item]);
    }
  }
...
// 搜索匹配统一谓词（最近模式与桶模式共用，避免最近模式下搜索失效）
...
// ================================================================
// 总览 modal + bind + 编辑 + 关联 + 配发
// currentFilterTag/currentlyCollapsed/currentData/avatarImages 经 getter/setter（whole-reassigned）。
// ================================================================
...
// ==================== 储藏间主 Modal 已抽至 modules/stash-modal.ts ====================
...
function bindManagedModalEvents(kind:ManagedKind, idPrefix:string) {
  ...
  qs('[data-fav-toggle]')?.addEventListener('click', function(this: HTMLElement) {
    ...
    if (grid) grid.innerHTML = renderManagedBuckets(kind, idPrefix, (qs<HTMLInputElement>(`#${idPrefix}-search`)?.value || '').trim().toLowerCase());
    bindManagedCardEvents(kind, idPrefix);
    bindCollapseToggleEvents(kind, idPrefix); // 收藏模式仍渲染桶分组,补绑折叠事件否则折叠按钮失灵
  });
...
async function openManagedEditModal(kind: ManagedKind, name: string) {
  ...
  setTimeout(() => {
    ...
    // AI 重写：弹 modal（人格/风格选择 + 额外输出指令），生成结果填回 desc 输入框（不自动保存）。
    qs('.th-btn-edit-ai-rewrite')?.addEventListener('click', () => {
      ...
      openRewriteModal(kind, name, curDesc, (text) => {
        const ta = qs<HTMLTextAreaElement>('textarea[data-edit-field="desc"]');
