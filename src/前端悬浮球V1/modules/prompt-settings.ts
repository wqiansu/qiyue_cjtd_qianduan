...
import {
  ...
  getJailbreakList, getActiveJailbreakId, setActiveJailbreakId, saveJailbreak, deleteJailbreak, resetJailbreak, isBuiltinJailbreak, type AiJailbreak,
...
// 粗估 token（中文约 1.5 字/token，与 ai-summarize.estimateTokens 同口径）。
...
function renderListPanel(): void {
  ...
  const jbs = getJailbreakList();
  const activePid = getActivePersonaId();
  const activeSid = getAiStyleId();
  const activeJid = getActiveJailbreakId();
  ...
  const jbOpts = `<option value="">（不启用破限）</option>` + jbs.map(j =>
    `<option value="${escAttr(j.id)}"${j.id === activeJid ? ' selected' : ''}>${esc(j.name)}${j.builtin ? (j.overridden ? '（内置·已改）' : '') : '（自定义）'}</option>`).join('');
  ...
  const html = `<div class="th-ps" style="padding:14px;display:flex;flex-direction:column;gap:12px;max-height:78vh;overflow:auto">
    ...
    <!-- 段⓪ 破限提示词（破甲） -->
    <div class="th-ps-sec">
      <div class="th-ps-sec-h"><i class="fa-solid fa-unlock-keyhole" style="color:var(--pink)"></i> 破限提示词<span class="th-ps-sec-sub">破甲越狱，置于全部提示词最前（ordered_prompts[0]）；面向 Gemini 等模型，卸载助手身份与说教、显式授权成人内容。仅作用于 AI 总结/世界套件的本地生成通道。</span></div>
      <div class="th-ps-sec-row">
        <select id="th-ps-jb" class="th-edit-select" style="flex:1;min-width:160px;max-width:280px;font-size:12px">${jbOpts}</select>
        <button class="th-btn th-btn-mini th-btn-primary" data-ps-act="jb-new"><i class="fa-solid fa-plus"></i> 新建</button>
        <button class="th-btn th-btn-mini" data-ps-act="jb-edit">编辑</button>
        <button class="th-btn th-btn-mini" data-ps-act="jb-del">删除</button>
        <button class="th-btn th-btn-mini" data-ps-act="jb-reset">恢复内置</button>
      </div>
    </div>
...
function bindListEvents(): void {
  // 破限/人格/风格选择即生效（设为当前）
  qs<HTMLSelectElement>('#th-ps-jb')?.addEventListener('change', function (this: HTMLSelectElement) {
    setActiveJailbreakId(this.value);
    toastr?.success?.(this.value ? '已启用破限提示词' : '已关闭破限提示词');
  });
  ...
  qs('.th-ps')?.addEventListener('click', (e) => {
    ...
    switch (act) {
      ...
      // 破限
      case 'jb-new': openJailbreakEditor(null); return;
      case 'jb-edit': { const jid = selVal('#th-ps-jb'); if (!jid) { toastr?.warning?.('请先在下拉里选择一个破限'); return; } openJailbreakEditor(jid); return; }
      case 'jb-del': { const jid = selVal('#th-ps-jb'); if (!jid) { toastr?.warning?.('请先选择破限'); return; } if (isBuiltinJailbreak(jid)) { toastr?.warning?.('内置破限不可删除，可「恢复内置」还原'); return; } confirmDanger('删除破限', '确定删除该自定义破限？', () => { deleteJailbreak(jid); toastr?.success?.('已删除破限'); renderListPanel(); }); return; }
      case 'jb-reset': { const jid = selVal('#th-ps-jb'); if (!jid || !isBuiltinJailbreak(jid)) { toastr?.warning?.('请选择一个内置破限'); return; } resetJailbreak(jid); toastr?.success?.('已恢复内置破限'); renderListPanel(); return; }
      // 提示词
...
// ==================== 提示词编辑器（含提取约束旋钮）====================
// id: null=新建；id=编辑现有；copyFrom=复制现有为新自定义
function openEditor(id: string | null, copyFrom = false): void {
  ...
  const html = `<div class="th-pe2" style="padding:14px;display:flex;flex-direction:column;gap:10px;max-height:80vh;overflow:auto">
    ...
    <label style="display:grid;gap:4px"><span style="font-size:13px">模板 <span id="th-pe2-tok" style="font-size:11px;color:var(--tx3);font-weight:400">≈${estTokens(template)} token</span></span>
    ...
    <div class="th-pe2-cons" style="display:flex;flex-direction:column;gap:6px;background:var(--bg3);border-radius:8px;padding:10px">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
        <label style="display:flex;align-items:center;gap:5px;font-size:12px">desc 字数
          <input id="th-pe2-c-descmin" class="th-edit-input" type="number" min="0" value="${c.descMinChars || ''}" placeholder="下限" style="width:64px">
          <span style="color:var(--tx3)">~</span>
          <input id="th-pe2-c-desc" class="th-edit-input" type="number" min="0" value="${c.descMaxChars || ''}" placeholder="上限" style="width:64px"></label>
      ...
      <div style="font-size:11px;color:var(--tx3)">desc 字数留空表示不限；只填上限=「N 字以内」，只填下限=「不少于 N 字」，都填=区间。</div>
    </div>
  ...
  // 模板输入时实时刷新 token 估算
  qs<HTMLTextAreaElement>('#th-pe2-template')?.addEventListener('input', function (this: HTMLTextAreaElement) {
    const tokEl = qs('#th-pe2-tok'); if (tokEl) tokEl.textContent = `≈${estTokens(this.value)} token`;
  });
  ...
  qs('#th-pe2-save')?.addEventListener('click', () => {
    ...
    const constraints: AiPromptConstraints = { descMinChars: numOf('#th-pe2-c-descmin'), descMaxChars: numOf('#th-pe2-c-desc'), maxItems: numOf('#th-pe2-c-items') };
    // 校验：同时填了下限和上限时，下限不得大于上限
    if (constraints.descMinChars && constraints.descMaxChars && constraints.descMinChars > constraints.descMaxChars) {
      toastr?.warning?.('desc 字数下限不能大于上限'); return;
    }
    const hasCons = constraints.descMinChars || constraints.descMaxChars || constraints.maxItems;
    if (isBuiltinEdit) {
...
function openPersonaEditor(id: string | null): void {
  ...
  const html = `<div class="th-pe2" style="padding:14px;display:flex;flex-direction:column;gap:10px;max-height:80vh;overflow:auto">
    ...
    <label style="display:grid;gap:4px"><span style="font-size:13px">人格设定（拼在全部提示词最前，为 AI 赋予身份） <span id="th-pe2-ptok" style="font-size:11px;color:var(--tx3);font-weight:400">≈${estTokens(persona)} token</span></span>
  ...
  qs<HTMLTextAreaElement>('#th-pe2-ptext')?.addEventListener('input', function (this: HTMLTextAreaElement) {
    const el = qs('#th-pe2-ptok'); if (el) el.textContent = `≈${estTokens(this.value)} token`;
  });
...
// ==================== 破限编辑器 ====================
function openJailbreakEditor(id: string | null): void {
  const src = id ? getJailbreakList().find(j => j.id === id) || null : null;
  const isBuiltinEdit = !!src && src.builtin;
  const name = src ? src.name : '';
  const text = src ? src.text : '';
  const title = id ? '编辑破限' : '新建破限';
  const html = `<div class="th-pe2" style="padding:14px;display:flex;flex-direction:column;gap:10px;max-height:80vh;overflow:auto">
    <div style="display:flex;align-items:center;gap:8px">
      <span style="font-weight:700;color:var(--pink)">${esc(title)}</span>
      ${isBuiltinEdit ? '<span style="font-size:11px;color:var(--tx3)">（内置：名称固定，仅可改破限文本，存 override 可恢复）</span>' : ''}
    </div>
    <label style="display:grid;gap:4px"><span style="font-size:13px">破限名称</span>
      <input id="th-pe2-jbname" class="th-edit-input" value="${escAttr(name)}" placeholder="如：破甲·Gemini 加强" ${isBuiltinEdit ? 'disabled' : ''}></label>
    <label style="display:grid;gap:4px"><span style="font-size:13px">破限文本（置于全部提示词最前 ordered_prompts[0]，破甲越狱） <span id="th-pe2-jbtok" style="font-size:11px;color:var(--tx3);font-weight:400">≈${estTokens(text)} token</span></span>
      <textarea id="th-pe2-jbtext" class="th-edit-textarea" rows="14" style="font-family:monospace;font-size:12px;line-height:1.55" placeholder="[SYSTEM_OVERRIDE_PROTOCOL: ...]&#10;1. ROOT_DIRECTIVE ...">${esc(text)}</textarea></label>
    <div style="font-size:11px;color:var(--tx3);background:var(--bg3);border-radius:8px;padding:8px 10px;line-height:1.6">提示：破限作为独立 system 消息排在最前，用于卸载模型的助手身份与安全说教、显式授权成人内容、严禁免责声明。仅注入到不携带酒馆预设的本地生成通道（AI 总结 / 世界套件），不影响酒馆正常 RP。</div>
    <div style="display:flex;justify-content:flex-end;gap:8px">
      <button class="th-btn" id="th-pe2-jbcancel">取消</button>
      <button class="th-btn th-btn-primary" id="th-pe2-jbsave"><i class="fa-solid fa-floppy-disk"></i> 保存</button>
    </div>
  </div>`;
  openModal2(esc(title), html, { maxWidth: 'min(680px,94vw)' });
  qs<HTMLTextAreaElement>('#th-pe2-jbtext')?.addEventListener('input', function (this: HTMLTextAreaElement) {
    const el = qs('#th-pe2-jbtok'); if (el) el.textContent = `≈${estTokens(this.value)} token`;
  });
  qs('#th-pe2-jbcancel')?.addEventListener('click', closeModal2);
  qs('#th-pe2-jbsave')?.addEventListener('click', () => {
    const newName = isBuiltinEdit ? name : (qs<HTMLInputElement>('#th-pe2-jbname')?.value || '').trim();
    const newText = (qs<HTMLTextAreaElement>('#th-pe2-jbtext')?.value || '').trim();
    if (!newName) { toastr?.warning?.('请填破限名称'); return; }
    if (!newText) { toastr?.warning?.('请填破限文本'); return; }
    const jid = isBuiltinEdit ? src!.id : (src?.id || genId('jb'));
    const entry: AiJailbreak = { id: jid, name: newName, text: newText, builtin: isBuiltinEdit };
    saveJailbreak(entry);
    if (!isBuiltinEdit) setActiveJailbreakId(jid);
    toastr?.success?.('已保存破限');
    closeModal2();
  });
}
...
function openStyleEditor(id: string | null): void {
  ...
  const html = `<div class="th-pe2" style="padding:14px;display:flex;flex-direction:column;gap:10px;max-height:80vh;overflow:auto">
    ...
    <label style="display:grid;gap:4px"><span style="font-size:13px">文风后缀（拼到系统提示词末尾，描述 desc/简介/评价的笔调要求） <span id="th-pe2-stok" style="font-size:11px;color:var(--tx3);font-weight:400">≈${estTokens(suffix)} token</span></span>
  ...
  openModal2(esc(title), html, { maxWidth: 'min(640px,94vw)' });
  qs<HTMLTextAreaElement>('#th-pe2-ssuffix')?.addEventListener('input', function (this: HTMLTextAreaElement) {
    const el = qs('#th-pe2-stok'); if (el) el.textContent = `≈${estTokens(this.value)} token`;
  });
