...
import { qs, qsa, esc } from '../lib/dom-utils';
import { thChoose } from '../lib/world/ui-kit';
...
import { openInitLayerVisualEditor } from './init-visual-editor';
...
type EntryState = {
  ...
  cardCount: number;                    // links=节点数；其余=卡片数
  health: Health;
  healthMsg: string;
};
...
type PanelState = {
  entries: EntryState[];
  rows: RowState[];                     // 三层 8 行看板（地点/事件/DLC/物品/技能/状态/衣物/关联）
...
function renderTriLayerDashboard(rows: RowState[]): string {
  ...
  return `<div class="th-init-sec">
    ...
    <div class="th-init-tri-head">
      ...
      <span class="th-init-tri-h-gap">⟷</span>
...
async function initToLive(kind: RowKind): Promise<void> {
  ...
  // 检测待导入卡片里「实时卡片标签字典中尚未建立」的标签。
  const existTags = loadTags()[kind as keyof ReturnType<typeof loadTags>] || {};
  const missingTags = new Set<string>();
  for (const [, item] of Object.entries(initMap)) {
    for (const tg of (item.tags || [])) { if (tg && !(existTags as Record<string, unknown>)[tg]) missingTags.add(tg); }
  }
  // 默认：保留卡片原标签（包括未建立的，渲染时兜底进未分类）。
  let tagMode: 'keep' | 'strip' | 'create' = 'keep';
  if (missingTags.size) {
    const choice = await thChoose({
      title: '存在未建立的标签',
      message: `待导入卡片包含 ${missingTags.size} 个本地未建立的标签：${[...missingTags].slice(0, 8).join('、')}${missingTags.size > 8 ? ' …' : ''}。要如何处理？`,
      options: [
        { value: 'create', label: '建立标签后导入', desc: '自动在实时卡片里创建这些标签，卡片归入对应分组', primary: true },
        { value: 'strip', label: '无标签导入', desc: '去掉未建立的标签，卡片进入「未分类」' },
        { value: 'keep', label: '保留原标签', desc: '原样保留（未建立的标签会归入未分类显示）' },
      ],
    });
    if (choice === null) return;   // 取消
    tagMode = choice as 'keep' | 'strip' | 'create';
    if (tagMode === 'create') {
      for (const tg of missingTags) { try { addTag(kind as any, tg, { color: TAG_COLOR_PALETTE[Math.floor(Math.random() * TAG_COLOR_PALETTE.length)], desc: '导入初始卡片时自动新建' }); } catch (e) { void e; } }
    }
  }
  ...
  for (const [name, item] of Object.entries(initMap)) {
    ...
    let tags = item.tags || [];
    if (tagMode === 'strip' && tags.length) tags = tags.filter(tg => (existTags as Record<string, unknown>)[tg]);
    addManagedItem(kind, name, { ...item, tags, favorite: false });
    added++;
  }
  if (added) toastr?.success?.(`已读入实时卡片：${ROW_LABEL[kind]} 补入 ${added}${skipped ? `，跳过 ${skipped}` : ''}${missingTags.size && tagMode === 'create' ? `，新建标签 ${missingTags.size}` : ''}`);
...
function editInitCards(kind: InitCardKind): void {
  // 可视化卡片网格编辑器（储藏间结构化字段拆分、地点/事件/DLC 带 links）。
  openInitLayerVisualEditor(
    kind,
    () => getInitCards(kind),
    (map) => setInitCards(kind, map),
    () => { void refresh(); },
  );
}
...
async function exportFullPack(): Promise<void> {
  try {
    ...
    // 纳入所有 _th_world_* key（去重，避免与上面重复）
    for (const k of getWorldStorageKeys()) if (!allKeys.includes(k)) allKeys.push(k);
