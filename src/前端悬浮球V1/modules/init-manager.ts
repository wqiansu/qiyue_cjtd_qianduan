...
// 世界套件 P0：整包导出纳入所有 _th_world_* key（套件配置 + 各 APP 数据 + 记忆动态 key）。
...
// 批次8 反馈6：数据载体（角色卡内嵌世界书）功能已删除——三层数据看板已覆盖其职责，char-book 导入随之移除。
import {
  type InitCardKind,
  INIT_CARD_KINDS,
  getInitCards,
...
function editInitCards(kind: InitCardKind): void {
  // 批次8 反馈5：改用可视化卡片网格编辑器（储藏间结构化字段拆分、地点/事件/DLC 带 links），不再用纯文本表单。
...
async function exportFullPack(): Promise<void> {
  try {
    ...
    // 世界套件 P0：纳入所有 _th_world_* key（去重，避免与上面重复）
