// AI 总结 schema + 解析容错 + 归一化
// 统一返回形状：{ results: [{ task_id, items: [...] }] }
...
// 去掉 tags/links/confidence —— AI 只返回 name + desc（+储藏间结构化字段），标签与关联由玩家手填。

// location/event 卡片结构（仅核心 name+desc，不生成 tags/links/confidence）
...
// AI 常把 JSON 夹在解释文字中间（前有寒暄、后有总结），整体解析必败。
...
export function parseAiResult(raw: unknown): ParsedAiResult {
  ...
  if (raw && typeof raw === 'object') {
    ...
    // 整体解析失败或为空时，从混合文本里提取所有平衡块逐个尝试（JSON 可能夹在文字中间）
...
// ==================== 归一化 ====================
// 按桶 kind 清洗。返回 { items } — items 是该桶所有任务的 items 合并。
// AI 不再生成 tags/links/confidence，归一化不再收集这些（标签/关联由玩家手填）。
...
// 供 doInject 把玩家手填的 links 归一化为 {locations,events,dlcs}（保留字段过滤能力，AI 路径不再用）。
export type LinksShape = { locations?: string[]; events?: string[]; dlcs?: string[] };
export function filterLinksShape(v: any): LinksShape | undefined {
  if (!v || typeof v !== 'object') return undefined;
  const out: LinksShape = {};
  const locs = toStrArr(v.locations);
  const evs = toStrArr(v.events);
  const dlcs = toStrArr(v.dlcs);
  if (locs.length) out.locations = locs;
  if (evs.length) out.events = evs;
  if (dlcs.length) out.dlcs = dlcs;
