...
// 反馈3：去掉 tags/links/confidence —— AI 只返回 name + desc（+储藏间结构化字段），标签与关联由玩家手填。
...
// ==================== 归一化（§E.7 #22，反馈3 去掉 confidence/links 收集）====================
// 按桶 kind 清洗。返回 { items } — items 是该桶所有任务的 items 合并。
// 反馈3：AI 不再生成 tags/links/confidence，归一化不再收集这些（标签/关联由玩家手填）。
