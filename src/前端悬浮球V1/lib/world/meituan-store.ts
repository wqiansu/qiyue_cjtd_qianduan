// 世界套件 —— 美团（meituan）数据层（meituan-store.ts）
// PC 端本地生活平台：外卖（商家→菜单→购物车→下单→骑手配送轨迹）+ 到店团购券（买券→我的券→核销）+
//   投喂/跑腿（给联系人点外卖/代购，跨 app 推微信）+ 会员签到霸王餐抽奖 + 红包神券 + 评价生态 +
//   骑手对话 + 钱包流水 + app 内通知（物流/客服）。数据纯本地 _th_world_meituan_v1。
// 分区(section)：附近/美食/奶茶甜点/食材买菜/买药/到店团购/私密配送（情趣·上门服务，吃双滑块）。
// 生态：城市烟火气 / 口味重口度 / 配送时效 / 商家活跃度 / 色情度(露骨程度) / 肉欲度(肉体肉欲与诱惑表现程度)。
