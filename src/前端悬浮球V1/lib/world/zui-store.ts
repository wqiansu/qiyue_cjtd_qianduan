// 世界套件 —— 最右（zui）数据层（zui-store.ts）
// 仙宫版「最右」：匿名短平快多模态搞笑 UGC 社区——段子/沙雕图/沙雕视频/表情包/灵魂画手/段子接龙，
//   核心灵魂＝「神评文化」（钓鱼楼/盖楼接龙/谐音空耳/神评 battle/挖坟考古）。
// 与 B站(视频)、糖心(直播)、微博(资讯广场)错开：这里是**匿名、抽象、玩梗、短平快**的乐子人乐园。
// 玩家＝乐子人：发帖(发条右)/围观/顶神评/接龙/收藏表情包/整活举报——涨沙雕值升级。
// 多模态铁律（对齐 xmly「用文字表现声音」）：图片/视频/表情包一律用 imageDesc 中文画面描述 + 文字表现，不接文生图。
// 全女性百合世界观，设定读绑定世界书。数据纯本地 _th_world_zui_v1。
