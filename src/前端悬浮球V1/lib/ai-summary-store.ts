...
// 增量模式。记录每条世界书条目上次发送时的内容 hash，下次内容未变更则跳过。
...
// task_id 由本层分配（addTask 时生成），用于统一发送时拼装 user_input 并回拆结果。
export type AiTask = {
  taskId: string;
  ...
  customInstruction: string;        // 玩家给本任务的单条备注指令（可空）
};
...
export function setLastDrillBook(book: string | null): void {
  if (book) localStorage.setItem(LS_LAST_DRILL, book); else localStorage.removeItem(LS_LAST_DRILL);
}
...
export const AI_STYLES_BUILTIN: AiStyle[] = [
  ...
  // 扩充题材风格（修仙/武侠/玄幻/西幻/科幻/赛博/古风/克苏鲁等），按题材调遣词与意象
...
// ==================== 破限提示词（破甲，置于全部提示词最前 ordered_prompts[0]）====================
// 内置来自 config.AI_JAILBREAKS_BUILTIN；自定义存 _th_ai_jailbreaks_v1；内置编辑存 override _th_ai_jb_overrides_v1。
// 当前选中破限 id 存 _th_ai_jb_active_v1（空 = 不启用破限）。与人格机制完全同构。
import { AI_JAILBREAKS_BUILTIN, type AiJailbreak } from './config';
export type { AiJailbreak } from './config';
const LS_JB_CUSTOM = '_th_ai_jailbreaks_v1';
const LS_JB_OVERRIDES = '_th_ai_jb_overrides_v1';
const LS_JB_ACTIVE = '_th_ai_jb_active_v1';

function readJbOverrides(): Record<string, string> {
  try { const o = JSON.parse(localStorage.getItem(LS_JB_OVERRIDES) || '{}'); return (o && typeof o === 'object' && !Array.isArray(o)) ? o : {}; } catch { return {}; }
}
function writeJbOverrides(m: Record<string, string>): void {
  try { localStorage.setItem(LS_JB_OVERRIDES, JSON.stringify(m)); } catch (e) { void e; }
}
function readCustomJbs(): AiJailbreak[] {
  try { const a = JSON.parse(localStorage.getItem(LS_JB_CUSTOM) || '[]'); return Array.isArray(a) ? a : []; } catch { return []; }
}
function writeCustomJbs(a: AiJailbreak[]): void {
  try { localStorage.setItem(LS_JB_CUSTOM, JSON.stringify(a)); } catch (e) { void e; }
}

export function getJailbreakList(): (AiJailbreak & { overridden?: boolean })[] {
  const ov = readJbOverrides();
  const builtins = AI_JAILBREAKS_BUILTIN.map(j => ({ ...j, text: ov[j.id] != null ? ov[j.id] : j.text, overridden: ov[j.id] != null }));
  const customs = readCustomJbs().map(j => ({ ...j, builtin: false }));
  return [...builtins, ...customs];
}
export function getActiveJailbreakId(): string {
  try { return localStorage.getItem(LS_JB_ACTIVE) || ''; } catch { return ''; }
}
export function setActiveJailbreakId(id: string): void {
  try { if (id) localStorage.setItem(LS_JB_ACTIVE, id); else localStorage.removeItem(LS_JB_ACTIVE); } catch (e) { void e; }
}
// 当前破限的文本（拼到 ordered_prompts[0]，即一切之前）；未启用返回空串。
export function getActiveJailbreakText(): string {
  const id = getActiveJailbreakId();
  if (!id) return '';
  const j = getJailbreakList().find(x => x.id === id);
  return j ? j.text : '';
}
export function saveJailbreak(j: AiJailbreak): void {
  if (AI_JAILBREAKS_BUILTIN.some(b => b.id === j.id)) {
    const m = readJbOverrides(); m[j.id] = j.text; writeJbOverrides(m);
  } else {
    const list = readCustomJbs(); const idx = list.findIndex(x => x.id === j.id);
    if (idx >= 0) list[idx] = j; else list.push(j); writeCustomJbs(list);
  }
}
export function deleteJailbreak(id: string): void {
  writeCustomJbs(readCustomJbs().filter(j => j.id !== id));
  if (getActiveJailbreakId() === id) setActiveJailbreakId('');
}
export function resetJailbreak(id: string): void {
  const m = readJbOverrides(); if (id in m) { delete m[id]; writeJbOverrides(m); }
}
export function isBuiltinJailbreak(id: string): boolean { return AI_JAILBREAKS_BUILTIN.some(j => j.id === id); }
