...
// 防数据丢失（关联双向同步等跨 kind 写入场景）：saveManagedOverrides 会把整份
// currentManagedItems[kind] 落盘覆盖 localStorage。若该 kind 的内存集从未被加载
// （例如只打开了 location 编辑 modal，却因双向关联去写 event），currentManagedItems
// ['event'] 还停留在初始 {}，单条写入后整库被残缺内存覆盖 → 其余卡片全被删。
// 这里在改单条前，先用持久化全集兜底，确保落盘的是「全集 + 本次改动」而非残缺内存。
...
// 锁定保护：切换卡片锁定态（锁定卡片 AI 注入时跳过，状态栏显🔒）
...
export function deleteStashKind(kindName: string): number {
  ...
  // 删除自定义类别时不丢数据——把卡片和标签定义迁移到固定 kind 'stash-uncategorized'
...
// ==================== 关联图 CRUD（location/event/dlc 三类 links 投影，纯数据层）====================
