...
// 必须是深拷贝快照：本地编辑会原地 mutate currentData，引用基线会被污染。
...
// 铁律：只记录「叶子」变更，绝不把整个对象/数组容器当成一条变更。
//   变量为 {A:A1, B:B1}，A1→A2 时应只出 `A: A1→A2` 一条，
//   不能冒出 `{A:A1,B:B1}→{A:A2,B:B1}` 这条整体结构 diff。
//   注意：不能要求「两侧都是 plain object」才下钻——一侧因表示差异（如 MVU 元组
//   [值,描述] 数组 vs 本地 plain 值、类型突变）不被识别为 plain object 时会把整块容器落一条。
//   故：只要有一侧是「容器」(对象或数组) 就按键/索引继续下钻，仅当两侧都是基元时才落一条叶子变更。
...
function walk(newObj: any, oldObj: any, prefix: string, out: ReviewItem[], ts: number, msgId: number): void {
  ...
  const newIsC = isContainer(newObj);
  const oldIsC = isContainer(oldObj);
  // 任一侧是容器 → 合并两侧的键/索引逐个下钻（缺失侧取 undefined），永不整块记录。
  if (newIsC || oldIsC) {
    const keys = new Set<string>([
      ...(newIsC ? Object.keys(newObj) : []),
      ...(oldIsC ? Object.keys(oldObj) : []),
    ]);
    // 空容器（如 {} / []）无键可下钻：视作无意义变更，跳过（不落整块 diff）。
    keys.forEach(k => {
      const childPath = prefix ? `${prefix}.${k}` : k;
      walk(newIsC ? _get(newObj, k) : undefined, oldIsC ? _get(oldObj, k) : undefined, childPath, out, ts, msgId);
    });
    return;
  }
  // 两侧都是基元 → 记录叶子变更。
