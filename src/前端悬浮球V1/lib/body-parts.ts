...
//  穿着部位解析（词素归一化）
//  ----------------------------------------------------------------
//  变量结构里「穿着部位」是开放 string，由 AI 每楼自由生成，措辞会漂移
//  （实测 V1 世界书 155 条 → 36 种写法）。但人体部位词素是封闭集合，
//  所以这里不做枚举匹配，只扫词素：层次前缀 + 部位词 + 侧向 + 复合拆分。
//  扫不到的一律进「其他」托盘，原文照挂，绝不猜、绝不丢。
//  人形图只是空间索引，衣物块列表始终是唯一真相，故允许不完备。
// ================================================================
