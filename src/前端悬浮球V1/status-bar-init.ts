...
// 世界套件 P0：「世界」按钮 → 桌面壳。各 APP 模块通过副作用 import 自注册到 world-store 注册表。
...
// 二级弹窗堆栈（批次5 Step1）：解决嵌套 modal 关闭后父级丢失。
...
function _modal2OverlayVisible(): boolean {
  const o = qs('.th-modal-overlay-2'); return !!o && o.style.display !== 'none';
}
export function openModal2(t:string,b:string,opts?:{maxWidth?:string;revive?:()=>void;replace?:boolean;reset?:boolean}){
...
export function closeModal2(){
  const overlay=qs('.th-modal-overlay-2');
...
function bindEvents() {
  ...
  // 世界套件 P0：顶栏「世界」按钮（审核↔菜单之间）→ 手机桌面壳
...
export async function setupStatusBar(): Promise<{ destroy: () => void }> {
  ...
  // 批次7 反馈6a：开机即挂世界书激活监听（面板未开也持续记录最近若干轮激活）
