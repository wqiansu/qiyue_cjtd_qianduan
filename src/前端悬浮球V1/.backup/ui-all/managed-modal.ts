...
// ==================== 渲染常量（B6 抽取，零行为变化）====================
const CARD_PREVIEW_LEN = 25;       // 卡片预览文本截断长度
...
function renderManagedCards(kind:ManagedKind, entries: [string, ManagedItemV2][], kindTags: Record<string, Tag>): string {
  ...
  return entries.map(([name, item])=>{
    ...
    const previewText = fullDisplayDesc.length > CARD_PREVIEW_LEN ? fullDisplayDesc.slice(0, CARD_PREVIEW_LEN) + '…' : fullDisplayDesc;
      ...
      h += `<div class="th-wb-row" style="align-items:flex-start;">
        <div style="display:flex;align-items:flex-start;gap:10px;flex:1;min-width:0;">
          ...
          <div style="min-width:0;flex:1;">
            ...
            <div style="font-size:12px;color:var(--tx3);margin-top:2px;line-height:1.5;word-break:break-word;">${esc(entry.content.slice(0, SCAN_CONTENT_PREVIEW))}${entry.content.length > SCAN_CONTENT_PREVIEW ? '...' : ''}</div>
