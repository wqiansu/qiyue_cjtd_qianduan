...
// ==================== 4b 第5项：卡片 AI 重写简介 ====================
// 跨窗口取 generateRaw（iframe→parent 兜底，同 ai-summarize 范式）
function getGenerateRawForRewrite(): ((cfg: any) => Promise<unknown>) | null {
  try {
...
// 反馈4：改用 generateRaw + ordered_prompts，系统提示词 = 头部人格 + 重写指令 + 风格后缀，
// 让卡片「AI 重写」跟随玩家在提示词设置里选的人格/风格（与 AI 总结、AI 润色同口径），不再各写各的。
async function aiRewriteDesc(kind: ManagedKind, name: string): Promise<void> {
  const generateRaw = getGenerateRawForRewrite();
  ...
  const sysPrompt = getActivePersonaText()
    + `你是一名游戏设定润色助手。请根据玩家给出的卡片名与当前简介，重写出一段简洁、生动、符合游戏设定的中文简介（不超过 80 字）。只输出简介正文，不要加标题、引号、解释或任何多余说明，不要用代码块标记包裹。忠于设定，不编造与原意冲突的硬事实。`
    + getAiStyleSuffix();
  const user_input = `【卡片类别】${cfg.label}\n【${cfg.label}名】${name}\n【当前简介】${curDesc || '(空)'}`;
  ...
  try {
    ...
    const raw = await generateRaw(genCfg);
