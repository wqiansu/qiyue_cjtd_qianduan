...
import { openTagManagerModal, openBatchTagSelectModal, setCurrentTagManagerKind, renderTagFilterBar } from './tag-manager';
import {
  type ParsedImport,
  downloadText,
...
// ==================== 桶折叠/展开辅助函数（Phase 2）====================
export function bindCollapseToggleEvents(kind: ManagedKind, idPrefix: string) {
...
// ==================== 按桶分组渲染卡片（Phase 2）====================
export function renderManagedBuckets(kind: ManagedKind, idPrefix: string, query: string = ''): string {
  const allTags = loadTags();
  const kindTags = allTags[kind] || {};
        ...
        case 'selectAll': {
          ...
          if (getCurrentFilterTag() !== null) {
            entries = entries.filter(([, item]) => {
              if (getCurrentFilterTag() === '') return !item.tags || item.tags.length === 0;
              return item.tags && item.tags.includes(getCurrentFilterTag());
            });
          }
...
async function openManagedEditModal(kind: ManagedKind, name: string) {
  ...
  setTimeout(() => {
    ...
    qs('.th-btn-add-to-user')?.addEventListener('click', () => {
      ...
      if (!getCurrentStatusData()) return;
      const userKey = getUserKey(getCurrentStatusData());
...
function openAddToNpcPopover(kind: ManagedKind, itemName: string, itemDesc: string, itemInject: string) {
  // 正确方式：用 getNPCs(getCurrentStatusData()) 获取 NPC 列表
  if (!getCurrentStatusData()) { toastr?.warning?.('当前没有数据'); return; }
  const allNpcs = getNPCs(getCurrentStatusData());
