...
// 本模块私有 whole-reassigned 状态（原主文件 currentSelectedTag / currentTagManagerKind）。
let currentSelectedTag: string | null = null;
let currentTagManagerKind: ManagedKind | null = null;
// 主文件批量打标入口（原 currentTagManagerKind = kind）需要从模块外部写入，故暴露 setter。
// 注：currentTagManagerKind 当前全局无读取方，仅保留写入以维持行为一致。
export function setCurrentTagManagerKind(kind: ManagedKind) { currentTagManagerKind = kind; }
