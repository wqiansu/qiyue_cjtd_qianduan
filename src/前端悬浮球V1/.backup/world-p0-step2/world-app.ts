...
function renderSettingsHtml(): string {
  ...
  return `<div class="th-world-settings" data-world-settings-root>
    ...
    <div class="th-world-set-group">
      <div class="th-world-set-glabel"><i class="fa-solid fa-database"></i> 记忆三层阈值</div>
      <label class="th-world-set-row">
        <span>短期：每 N 条对话触发小总结</span>
        <input type="number" min="1" class="th-world-set-mem-short" value="${esc(String(cfg.memory.shortThreshold))}">
      </label>
      <label class="th-world-set-row">
        <span>长期：每 N 条小总结压缩为大总结</span>
...
function saveSettingsFromForm(root: HTMLElement): void {
  ...
  const shortRaw = Number((root.querySelector('.th-world-set-mem-short') as HTMLInputElement | null)?.value);
  const longRaw = Number((root.querySelector('.th-world-set-mem-long') as HTMLInputElement | null)?.value);
  const shortThreshold = Number.isFinite(shortRaw) && shortRaw >= 1 ? Math.floor(shortRaw) : getWorldConfig().memory.shortThreshold;
  const longThreshold = Number.isFinite(longRaw) && longRaw >= 1 ? Math.floor(longRaw) : getWorldConfig().memory.longThreshold;
  saveWorldConfig({
    comfyui: { enabled, url: url || 'http://127.0.0.1:8188', workflowJson: wf },
    memory: { shortThreshold, longThreshold },
  });
