...
【输出格式】严格按下方 JSON 格式输出，不要输出任何多余文字、解释或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"黑森林","desc":"终年不见阳光的密林","tags":["危险","自然"],"links":{"locations":["王都"],"events":["森林遇袭"]}}]}]}
多条示例（items 数组放多个元素，task_id 见上方各任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"黑森林","desc":"终年不见阳光的密林","tags":["危险"]},{"name":"王都","desc":"王国的政治中心","tags":["都市"]}]}]}`,
  },
  {
...
【输出格式】严格按下方 JSON 格式输出，不要输出任何多余文字、解释或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"森林遇袭","desc":"主角在黑森林遭遇狼群","tags":["战斗"],"links":{"locations":["黑森林"]}}]}]}
多条示例（items 数组放多个元素，task_id 见上方各任务块）：
