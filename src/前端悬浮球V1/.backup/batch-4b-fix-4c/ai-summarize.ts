...
function renderPanel(): void {
  ...
  <div class="th-ai-sum" style="padding:14px;display:flex;flex-direction:column;gap:12px">
    <div class="th-ai-cols" style="display:grid;grid-template-columns:1fr 1fr;gap:14px;min-height:440px">
      ${renderSelector()}
      ${renderTaskpool()}
    </div>
...
function buildBucketInput(bucket: Bucket, contents: Map<string, string>, prompt: AiSummaryPrompt): string {
  ...
  // 4b 第1项：统一追加置信度自评要求（覆盖内置+自定义全部提示词，不逐个改模板）
  const confReq = `\n【附加要求】对每个提取项，请在 confidence 字段给出你对本次提取把握的自评：原文明确写出的填「高」，需合理推断的填「中」，把握很低或存疑的填「低」。`;
  return `${tmpl}\n\n${blocks.join('\n\n')}${confReq}`;
}
...
async function onSend(): Promise<void> {
  ...
  openModal2('AI 总结中', `<div class="th-ai-progress" style="padding:18px;display:grid;gap:10px">
    ...
    <div id="th-ai-prog-stream" style="font-size:11px;color:var(--tx2);background:var(--bg2);border-radius:8px;padding:8px 10px;max-height:140px;overflow:auto;font-family:monospace;white-space:pre-wrap;min-height:24px"></div>
  ...
  for (let i = 0; i < buckets.length; i++) {
    ...
    setStream('');
    const r = await sendOneBucket(b, contents, prompt, cfg, generate);
    results.push(r);
    if (r.ok) {
      // 4b 第4项：记录本桶各条目内容 hash，供下次增量判断（仅成功桶才记录，失败桶下次重试）
      if (getIncrEnabled()) {
        for (const t of b.tasks) recordIncrSent(t.book, t.entryName, contents.get(t.taskId) || '');
      }
      appendLog(`✓ ${prompt?.label || b.kind}：提取 ${r.items.length} 项`);
...
function setStream(text: string): void {
  ...
  if (el) el.textContent = text || '（等待 AI 输出…）';
}
...
async function rerollBucket(kind: AiSummaryPromptKind, btn: HTMLButtonElement): Promise<void> {
  ...
  const newRes = await sendOneBucket(bucket, LAST_SEND.contents, prompt, cfg, generate);
