...
function bindPanelEvents(): void {
  ...
  // 世界书进入/返回/条目勾选/全选反选（事件委托）
  qs('#th-ai-back')?.addEventListener('click', () => { ST.drillBook = null; rerenderSelector(); });
...
async function onSearch(q: string): Promise<void> {
  ...
  // 条目视图：只过滤本书条目（renderBookEntries 已处理），直接重渲染
  if (ST.drillBook != null) { rerenderSelector(); return; }
  // 书列表视图：需加载各书条目以判断命中
...
function buildBucketInput(bucket: Bucket, contents: Map<string, string>, prompt: AiSummaryPrompt): string {
  ...
  if (c) {
    const lines: string[] = [];
    if (c.descMaxChars && c.descMaxChars > 0) lines.push(`每项 desc/简介控制在 ${c.descMaxChars} 字以内`);
    if (c.maxItems && c.maxItems > 0) lines.push(`单个任务最多提取 ${c.maxItems} 个条目（按重要性取前 ${c.maxItems} 个）`);
    if (c.attrTotalCap && c.attrTotalCap > 0) lines.push(`涉及数值/属性时，单项各数值之和不超过 ${c.attrTotalCap}`);
