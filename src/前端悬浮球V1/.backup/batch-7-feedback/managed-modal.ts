...
// 4b 第5项：卡片 AI 重写 — 复用 AI 总结的 generate 调用配置
...
import { openRefineModal } from './ai-refine';
import { openExportToWorldbookModal } from './card-export';
    ...
    return `<div class="${cls}" data-managed-kind="${kind}" data-managed-name="${escAttr(name)}" data-managed-desc="${escAttr(desc)}" title="${escAttr(title)}">
      ...
      <div class="th-managed-card-main" style="flex:1;min-width:0">
        <div class="th-managed-card-title" style="font-weight:700;font-size:14px;line-height:1.3">
          ...
          ${item.locked ? ' <i class="fa-solid fa-lock th-lock-badge" title="已锁定：AI 注入会跳过此卡片" style="font-size:10px;color:var(--gold);margin-left:4px"></i>' : ''}
...
// ==================== 批次1：双向关联同步 helper（从编辑保存闭包抽出，供任务1/3 复用）====================
// 行为与原 managed-modal 编辑保存内联 syncBidirectionalLinks 完全一致（行为零变化）。
// kind = 当前正在编辑的卡片 kind；name = 该卡片名；newLinks = 编辑后选中的关联；oldLinks = 编辑前旧关联。
// - 新增关联：给目标卡片补反向（A→B 新增则 B→A 补）
// - 移除关联：给目标卡片移除反向
// - 保留关联：补全历史单向缺失（A→B 在但 B→A 缺则补写）
export function syncBidirLink(kind: ManagedKind, name: string, newLinks: { locations?: string[]; events?: string[]; dlcs?: string[] }, oldLinks?: { locations?: string[]; events?: string[]; dlcs?: string[] }): void {
  const selfField = LINKS_KIND_FIELDS[kind as 'location' | 'event' | 'dlc']; // location→locations, event→events, dlc→dlcs
  const old = oldLinks || {};
  for (const field of ['locations', 'events', 'dlcs'] as const) {
    const targetKind = field === 'locations' ? 'location' : field === 'events' ? 'event' : 'dlc';
    const oldArr = old[field] || [];
    ...
    for (const targetName of newArr) {
      if (!oldArr.includes(targetName)) {
        ...
        if (targetItems[targetName]) {
          ...
          if (!targetFieldArr.includes(name)) {
            targetFieldArr.push(name);
    ...
    for (const targetName of oldArr) {
      if (!newArr.includes(targetName)) {
        ...
        if (targetItems[targetName]) {
          const targetLinks = targetItems[targetName].links || {};
          const targetFieldArr = targetLinks[selfField] || [];
          const idx = targetFieldArr.indexOf(name);
          if (idx >= 0) {
...
function bindManagedModalEvents(kind:ManagedKind, idPrefix:string) {
  ...
  // 批次6 反馈5：「重读初始数据🌱/写入初始数据」按钮已移除，统一收拢到「初始化管理」面板。
...
async function openManagedEditModal(kind: ManagedKind, name: string) {
  ...
  const h = `
    <div class="th-managed-edit-modal" data-kind="${kind}" data-name="${escAttr(name)}">
      ...
      <div class="th-modal-section">
        <div class="th-modal-label">简介 <button class="th-btn-sm th-btn-edit-ai-rewrite" type="button" title="用 AI 根据卡片名重写简介" style="margin-left:8px;padding:1px 8px;font-size:11px"><i class="fa-solid fa-wand-magic-sparkles"></i> AI 重写</button></div>
      ...
      <div class="th-edit-actions">
        ...
        <button class="th-btn-sm th-btn-edit-refine" type="button" title="调 AI 润色当前描述"><i class="fa-solid fa-wand-magic-sparkles"></i> AI 润色</button>
        <button class="th-btn-sm th-btn-edit-export-wb" type="button" title="把此卡片导出为世界书条目"><i class="fa-solid fa-file-export"></i> 导出条目</button>
        <button class="th-btn-sm th-btn-edit-lock" type="button" title="锁定后 AI 注入会跳过此卡片，防止被覆盖"><i class="fa-solid fa-${item.locked ? 'lock' : 'lock-open'}"></i> ${item.locked ? '已锁定' : '锁定'}</button>
  ...
  setTimeout(() => {
    ...
    // Step6 C：锁定/解锁开关（锁定后 AI 注入跳过此卡片）
    qs('.th-btn-edit-lock')?.addEventListener('click', function (this: HTMLElement) {
      const locked = toggleLock(kind, name);
      const icon = this.querySelector('i');
      if (icon) icon.className = `fa-solid fa-${locked ? 'lock' : 'lock-open'}`;
      this.innerHTML = `<i class="fa-solid fa-${locked ? 'lock' : 'lock-open'}"></i> ${locked ? '已锁定' : '锁定'}`;
      toastr?.info?.(locked ? '已锁定，AI 注入将跳过此卡片' : '已解锁');
    });
    // Step7 G：AI 润色（润色后回主 modal 并恢复状态/重渲染）
    qs('.th-btn-edit-refine')?.addEventListener('click', () => {
      openRefineModal(kind, name, () => { restoreMainModalState(); });
    });
    // Step7 F：导出为世界书条目
    qs('.th-btn-edit-export-wb')?.addEventListener('click', () => {
      openExportToWorldbookModal(kind, [name]);
    });
    ...
    // 批次1：核心逻辑抽到模块级 syncBidirLink helper（供任务1/3 复用），此处薄包装保持调用点不变（行为零变化）
    function syncBidirectionalLinks(itemName: string, newLinks: { locations: string[]; events: string[]; dlcs: string[] }) {
      ...
      syncBidirLink(kind, itemName, newLinks, oldLinks);
    }

    // 4b 第5项：AI 重写简介 — 喂卡片名+当前简介，让 AI 重写后填回 desc 输入框（不自动保存，玩家可再改）
    qs('.th-btn-edit-ai-rewrite')?.addEventListener('click', () => {
      void aiRewriteDesc(kind, name);
    });
...
// ==================== 4b 第5项：卡片 AI 重写简介 ====================
// 跨窗口取 generate（iframe→parent 兜底，同 ai-summarize 范式）
function getGenerateForRewrite(): ((cfg: any) => Promise<unknown>) | null {
  try {
...
// 用 AI 重写卡片简介：读当前编辑框里的 name+desc → 生成新简介 → 填回 desc 输入框（不自动保存）。
// 不走 json_schema，让 AI 自由输出一段中文简介；结果做最小清洗（去 ``` 围栏/首尾引号/空白）。
async function aiRewriteDesc(kind: ManagedKind, name: string): Promise<void> {
  const generate = getGenerateForRewrite();
  if (!generate) { toastr?.error?.('当前环境无 generate 接口'); return; }
  const cfg = kind.startsWith('stash-') ? getStashKindCfg(kind) : MANAGED_CFG[kind];
  const descTextarea = qs<HTMLTextAreaElement>('textarea[data-edit-field="desc"]');
  const curDesc = descTextarea?.value || '';
  const user_input = `请重写以下${cfg.label}卡片的「简介」字段，输出一段简洁、生动、符合游戏设定的中文简介（不超过80字），只输出简介正文，不要加标题、引号或多余说明。
【${cfg.label}名】${name}
【当前简介】${curDesc || '(空)'}`;
  const btn = qs<HTMLButtonElement>('.th-btn-edit-ai-rewrite');
  const old = btn?.innerHTML;
  if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 重写中…'; }
  toastr?.info?.('AI 重写中…');
  try {
    const apiCfg = resolveGenerateApiConfig();
    const genCfg: any = { user_input, should_silence: true };
    if (apiCfg.custom_api) genCfg.custom_api = apiCfg.custom_api;
    if (apiCfg.preset_name && apiCfg.preset_name !== 'in_use') genCfg.preset_name = apiCfg.preset_name;
    const raw = await generate(genCfg);
    let text = typeof raw === 'string' ? raw : (raw && typeof raw === 'object') ? String((raw as any).message ?? (raw as any).results ?? raw) : String(raw ?? '');
    // 去代码块围栏 + 首尾引号 + 空白
    text = text.replace(/^```(?:json|text)?\s*/i, '').replace(/\s*```$/i, '').trim();
    text = text.replace(/^["""']+|["""']+$/g, '').trim();
    if (!text) { toastr?.warning?.('AI 未返回有效内容'); return; }
    if (descTextarea) { descTextarea.value = text; descTextarea.dispatchEvent(new Event('input')); }
    toastr?.success?.('AI 重写完成，可在保存前再调整');
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    toastr?.error?.(`AI 重写失败：${msg}`);
  } finally {
    if (btn && old) { btn.disabled = false; btn.innerHTML = old; }
  }
}
