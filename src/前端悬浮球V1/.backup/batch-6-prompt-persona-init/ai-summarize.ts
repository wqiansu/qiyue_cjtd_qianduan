...
function freshState(): PanelState {
  ...
  return { books: listWorldbooks(), expanded: new Set(), selectedEntries: new Set(), promptId: prompts[0]?.id ?? '', searchQ: '', searchHits: new Set() };
}
...
function renderSelector(): string {
  ...
  const bookRows = ST.books.map(b => {
    ...
    const hitCnt = ST.searchHits.size ? b.entries?.filter(e => ST.searchHits.has(`${b.name}::${e.name}`)).length || 0 : 0;
  ...
  return `<div class="th-ai-sel" style="display:flex;flex-direction:column;gap:10px;min-height:0">
    ...
    <input id="th-ai-search" class="th-edit-input" style="width:100%;font-size:12px" placeholder="搜索条目/世界书名（跨所有书过滤，命中自动展开高亮）" value="${escAttr(ST.searchQ)}">
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
...
function bindPanelEvents(): void {
  ...
  // 反馈1：搜索框 — 跨所有书过滤条目/世界书名，命中自动展开高亮（防抖）
  let searchTimer: ReturnType<typeof setTimeout> | null = null;
  qs('#th-ai-search')?.addEventListener('input', (e) => {
...
// 反馈1：搜索 — 跨所有书过滤条目名/世界书名，命中自动展开高亮；未加载的书按需 await 加载。
async function onSearch(q: string): Promise<void> {
  const query = q.trim().toLowerCase();
  ST.searchHits = new Set();
  if (!query) { rerenderSelector(); return; }
  const books = ST.books.filter(b =>
    b.name.toLowerCase().includes(query) || (b.source && b.source.toString().toLowerCase().includes(query)));
  // 全书名命中 → 全书条目都算命中；否则逐条匹配
  for (const b of ST.books) if (!books.includes(b)) books.push(b);
  ...
  // 命中条目所在书自动展开，无命中的书收起（更聚焦）
  ST.expanded = new Set();
  for (const key of ST.searchHits) {
    const book = key.split('::')[0];
    ST.expanded.add(book);
  }
  rerenderSelector();
}
...
async function sendOneBucket(
  ...
  try {
    ...
    const genCfg: any = {
      ...
      ordered_prompts: [
        { role: 'system', content: AI_SUMMARY_SYSTEM_PROMPT + getAiStyleSuffix() },
        'user_input',
      ],
