...
// 作为专业提示词工程：交代角色、卡片体系与各 kind 字段、JSON 输出契约、防幻觉关联、置信度自评、纯 JSON 约束。
...
- 地点(location)/事件(event)：name（名称）、desc（一句话简介，客观简明，≤40字）、tags（标签数组，可选）、links（关联，可选，对象{locations,events,dlcs}，每个是名称数组）、confidence（把握自评，可选）
- 物品(stash-item)：name、数量（整数，默认1）、简介、效果、评价、confidence
- 技能(stash-skill)：name、等级（整数，默认1）、简介、效果、评价、confidence
- 状态(stash-status)：name、效果、来源、持续时间、confidence
- 衣物(stash-clothing)：name、穿着部位、穿着情况（仅"穿着"或"脱下"）、破损状态（仅"完好无缺"/"轻微破损"/"中度破损"/"严重破坏"之一）、外观详情、衣物状态、评价、confidence

【提取规则】
1. 只提取原文明确出现的要素；原文未提及的字段留空字符串或默认值（数量/等级默认1，穿着情况默认"穿着"，破损状态默认"完好无缺"），不要编造。
2. desc/简介等文字字段要精炼客观，不要夹带主观臆测；评价字段可适度风趣但需基于原文。
3. links 关联：只填原文明确点名的其他地点/事件/DLC 名称；拿不准的不要填，宁可留空也不要幻觉关联。
4. confidence（把握自评）：原文明确写出的填"高"，需合理推断的填"中"，把握很低或存疑的填"低"。
...
- tags：可选标签数组（如 危险/自然/都市/秘境/圣地），从原文归纳。
- links：可选，仅填原文明确点名的关联地点/事件/DLC 名（对象 {locations,events,dlcs}，每个为名称数组），拿不准的不填。
- confidence：把握自评（高/中/低）。
...
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"黑森林","desc":"终年不见阳光的密林","tags":["危险","自然"],"links":{"locations":["王都"],"events":["森林遇袭"]},"confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"黑森林","desc":"终年不见阳光的密林","tags":["危险"],"confidence":"高"},{"name":"王都","desc":"王国的政治中心","tags":["都市"],"confidence":"高"}]}]}`,
  },
  {
...
- tags：可选标签数组（如 战斗/日常/主线/支线/隐藏），从原文归纳。
- links：可选，仅填原文明确点名的关联地点/事件/DLC 名（对象 {locations,events,dlcs}），拿不准的不填。
- confidence：把握自评（高/中/低）。
...
【输出格式】严格按下方 JSON 输出，不要任何多余文字或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"森林遇袭","desc":"主角在黑森林遭遇狼群","tags":["战斗"],"links":{"locations":["黑森林"]},"confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"森林遇袭","desc":"主角在黑森林遭遇狼群","confidence":"高"},{"name":"王都夜宴","desc":"国王举办的盛大晚宴","confidence":"中"}]}]}`,
  },
  {
...
【输出格式】严格按下方 JSON 输出，不要任何多余文字或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"治疗药水","数量":2,"简介":"恢复体力的红色药水","效果":"恢复50点体力","评价":"常见消耗品","confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"治疗药水","数量":2,"简介":"恢复体力的红色药水","效果":"恢复50点体力","评价":"常见消耗品","confidence":"高"},{"name":"铁剑","数量":1,"简介":"制式铁剑","效果":"造成20点伤害","评价":"基础武器","confidence":"高"}]}]}`,
  },
  {
...
【输出格式】严格按下方 JSON 输出，不要任何多余文字或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"火球术","等级":3,"简介":"释放火球的初级法术","效果":"造成80点火焰伤害","评价":"输出主力","confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"火球术","等级":3,"简介":"释放火球的初级法术","效果":"造成80点火焰伤害","评价":"输出主力","confidence":"高"},{"name":"治愈术","等级":2,"简介":"恢复生命的法术","效果":"恢复40点生命","评价":"续航技能","confidence":"高"}]}]}`,
  },
  {
...
【输出格式】严格按下方 JSON 输出，不要任何多余文字或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"中毒","效果":"每回合扣除10点生命","来源":"毒蛇咬击","持续时间":"3回合","confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"中毒","效果":"每回合扣除10点生命","来源":"毒蛇咬击","持续时间":"3回合","confidence":"高"},{"name":"兴奋","效果":"攻击力提升20%","来源":"勇气药水","持续时间":"5回合","confidence":"中"}]}]}`,
  },
  {
...
【输出格式】严格按下方 JSON 输出，不要任何多余文字或代码块标记（不要用 \`\`\`json 包裹）。
单条示例：
{"results":[{"task_id":"task-1","items":[{"name":"丝绸长裙","穿着部位":"下身","穿着情况":"穿着","破损状态":"完好无缺","外观详情":"白色丝绸长裙","衣物状态":"洁净","评价":"优雅","confidence":"高"}]}]}
多条示例（items 放多个元素，task_id 对应用户输入中的任务块）：
{"results":[{"task_id":"task-1","items":[{"name":"丝绸长裙","穿着部位":"下身","穿着情况":"穿着","破损状态":"完好无缺","外观详情":"白色丝绸长裙","衣物状态":"洁净","评价":"优雅","confidence":"高"},{"name":"皮靴","穿着部位":"足部","穿着情况":"脱下","破损状态":"轻微破损","外观详情":"棕色皮靴","衣物状态":"沾泥","评价":"耐穿","confidence":"中"}]}]}`,
  },
];
