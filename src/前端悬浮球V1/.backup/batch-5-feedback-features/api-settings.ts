...
// AI 总结来源区块（反馈修复 2026-06-24）：选一个已保存的 API 预设 + 选提示词预设 + 仅本次有效；
...
interface PanelState {
  ...
  testing: boolean;
  // 反馈修复 2026-06-24：AI 总结来源 — 选哪个已保存 API 预设 + 提示词预设 + 仅本次有效
...
function renderApiPanel(): string {
  ...
  return `<div class="th-api">
    ...
    <div class="th-api-hint"><i class="fa-solid fa-circle-info"></i> API 连接 + 获取模型已可用；AI 总结来源（选择 API 预设 × 提示词预设）可保存，批次4 AI 总结据此调用。</div>
  </div>`;
}
