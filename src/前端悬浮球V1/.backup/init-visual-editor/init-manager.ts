...
export async function openInitManager(): Promise<void> {
  ...
  openModal2(`<i class="fa-solid fa-database"></i>初始化管理`, html, { maxWidth: 'min(820px,94vw)' });
  ...
  const rows = entries.map(e => {
    ...
    return `<div class="th-init-row" data-init-key="${e.key}">
      <div class="th-init-row-main">
        ...
        <div class="th-init-row-meta"><span class="th-init-tag">${esc(e.label)}</span><span class="th-init-books" title="${escAttr(books)}"><i class="fa-solid fa-book"></i> ${books}</span><span class="th-init-count">${cardCnt}</span></div>
        ${renderHealthRow(e)}
      </div>
      <div class="th-init-row-actions">
        <button class="th-btn-xs" data-init-act="read" type="button"><i class="fa-solid fa-arrow-down-to-line"></i> 读取</button>
        <button class="th-btn-xs" data-init-act="write" type="button"><i class="fa-solid fa-arrow-up-from-line"></i> 写入</button>
        <button class="th-btn-xs" data-init-act="edit" type="button"><i class="fa-solid fa-pen"></i> 编辑</button>
      </div>
    </div>`;
  }).join('');
  return `<div class="th-init-sec">
    <div class="th-init-sec-title"><i class="fa-solid fa-table-list"></i> 内容看板<span class="th-init-sec-sub">5 个初始数据条目</span></div>
...
function openRestoreModal(backup: any): void {
  ...
  setTimeout(() => {
    qs('#th-init-restore-cancel')?.addEventListener('click', () => { closeModal2(); void openInitManager(); });
    qs('#th-init-restore-confirm')?.addEventListener('click', async () => {
      ...
      try {
        ...
        await openInitManager();
      } catch (e) { console.warn('[init-manager]恢复失败', e); toastr?.error?.('恢复失败：' + (e as Error).message); void openInitManager(); }
    });
  }, 40);
}
