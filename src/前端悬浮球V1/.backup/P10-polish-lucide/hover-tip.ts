...
// ================================================================
import { esc, qs, clamp, editableInput, editableTextarea, __doc, __body } from '../lib/dom-utils';
...
import { openModal, openModal2, getStatusEditMode } from '../status-bar-init';
...
export function showHoverTip(anchor:HTMLElement, type:string, data:Record<string,string>) {
  ...
  if(type==='status'){
    ...
    html=`<div class="th-hover-tip-title"><i class="fa-solid fa-heart-circle-plus"></i> 交互记录</div><div class="th-hover-tip-row"><i class="fa-solid fa-sparkles"></i> 顶峰次数: <b>${esc(data.peak||'0')}</b></div><div class="th-hover-tip-row"><i class="fa-solid fa-water"></i> 深入次数: <b>${esc(data.deep||'0')}</b></div>`;
  }
...
export function showClothingHoverTip(anchor:HTMLElement, clothing:Record<string,any>) {
  if(hoverTimeout) clearTimeout(hoverTimeout);
  const tip=ensureHoverTip(); if(!tip) return;
  // 清理旧类型类
  tip.className = 'th-hover-tip';
...
export function openClothingDetailModal(name:string, part:string, state:string, detail:string, editPath:string) {
  let h='';
  if(getStatusEditMode()){
    h+=`<div class="th-modal-section"><div class="th-modal-label">名称</div>${editableInput(name,editPath+'.名称')}</div>`;
