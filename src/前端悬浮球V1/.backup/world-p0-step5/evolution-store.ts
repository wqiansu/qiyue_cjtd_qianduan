...
export const DEFAULT_EVO_CONFIG: EvoConfig = { aiPresetName: '', readFloors: 0, injectRecent: 3, maxBatch: 6 };
...
export function ensureActor(a: { source: EvoSource; sourceRef?: string; name: string; persona?: string; dimension?: string }): EvoActor {
  const d = read();
  ...
  const created: EvoActor = {
    ...
    persona: a.persona, dimension: a.dimension, injectEnabled: false, timeline: [], createdAt: t, updatedAt: t,
  };
...
// #3 更新某对象的「角色配置」（专属世界书条目 / 额外设定 / 人设 / 维度）。不动时间线。
export function updateActorConfig(id: string, patch: Partial<Pick<EvoActor, 'persona' | 'worldbookRefs' | 'extraNote' | 'dimension' | 'name'>>): void {
  const d = read();
