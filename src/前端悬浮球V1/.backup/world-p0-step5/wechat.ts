...
registerPromptTemplate({
  ...
  desc: '单聊时角色如何回复你。控制「像真人发微信、拆成多条短消息」的风格。',
  vars: [{ key: 'maxBubbles', desc: '本轮最多几条消息气泡' }, { key: 'name', desc: '角色昵称' }],
  default: '现在的你，就是「{{name}}」，正握着手机和「我」用微信聊天——不是在表演，而是真的在过你自己的生活。\n'
    + '请完全沉浸进「{{name}}」此刻的身份、心情和你们之间的关系里：你今天可能累、可能开心、可能心里揣着事，这些都会渗进你打字的语气。\n'
    + '像真人发微信那样回：想到哪说到哪，把话拆成 1~{{maxBubbles}} 条短消息，一条一句、有先有后；可以有口头禅、语气词、错字改口、突然想起补一句的那种真实感。\n'
    + '该热情就热情、该敷衍就敷衍、该撒娇/吐槽/沉默就照你的人设来，别讨好、别端着、别像客服。\n'
    + '不要写成一大段长文，不要旁白、不要动作神态描写、不要括号里的心理活动——微信里只有你打出来的字。\n'
    + '严格只输出 JSON：{"messages":["第一条","第二条", ...]}，除此之外不要任何文字。',
});
registerPromptTemplate({
  id: 'wechat.group', appId: 'wechat', appName: '微信', name: '群聊回复',
  desc: '群聊时多位成员如何接话（一轮可多人、每人可多条）。',
  vars: [{ key: 'maxSpeakers', desc: '本轮最多几位成员发言' }, { key: 'maxBubbles', desc: '每位成员最多几条气泡' }],
  default: '这是一个微信群，群里这些人都是活生生的、各有各脾气的朋友，此刻都在线、都瞥着手机。\n'
    + '请让他们像真人在群里那样自然接话：本轮可由 1~{{maxSpeakers}} 个人冒泡，谁性子急谁先抢话、谁慢热谁后补刀；每个人发 1~{{maxBubbles}} 条短消息，一条一句、口语化。\n'
    + '群聊的灵魂是「互相」：可以接梗、起哄、拌嘴、@对方、跑题、玩梗、突然安静又突然炸出来——让对话有来有回、有节奏、有温度，而不是各说各的轮流播报。\n'
    + '每个人说的话都要贴死自己的人设和当下心情，关系好的损得亲、关系生的客气点。\n'
...
registerPromptTemplate({
  ...
  default: '以「{{name}}」的身份，发一条此刻真想发的朋友圈。\n'
    + '想想你今天经历了什么、心里在想什么——是炫耀、是emo、是分享、是阴阳怪气、还是只想留个记号，全凭你此刻的状态。\n'
    + '用你自己的说话方式写，第一人称，60 字以内，像真实朋友圈那样有生活气：可以带点情绪、带点小心思、带点欲言又止，可适度用 emoji 或省略号，但别滥用。\n'
    + '直接给正文本身，不要引号、不要标题、不要解释、不要旁白。',
});
registerPromptTemplate({
  ...
  default: '下面有一条朋友圈，这些人刷到了，各自想冒泡评论一句。\n'
    + '请让每个人按自己的人设和跟发圈者的关系来评：要好的可以调侃、玩梗、戳痛处；不熟的客气点；爱凑热闹的接梗、爱关心的追问。\n'
    + '每句 30 字内，口语化、有那个人的味儿，别像群发祝福。参与者及人设：\n{{roster}}\n\n'
    + '这条朋友圈内容是：{{moment}}\n\n'
...
type SheetState =
  ...
  | { kind: 'compose'; chatId: string; mode: 'image' | 'desc' }
...
function render(): void {
  ...
  if (_view.name === 'chat' && _view.chatId) {
    ...
    if (chat) ensureSession({ id: wxSessionId(_view.chatId), appId: 'wechat', appName: '微信', title: chat.name });
    scrollMsgsBottom();
  }
}
...
function listHtml(): string {
  ...
  return `<div class="th-wx-view">
    <div class="th-wx-tabbar">
      <button class="th-wx-tab th-wx-tab-on" data-wx-go="list" type="button">${iconHtml('fa-comment-dots')} 聊天</button>
      <button class="th-wx-tab" data-wx-go="contacts" type="button">${iconHtml('fa-id-card')} 通讯录</button>
      <button class="th-wx-tab" data-wx-go="moments" type="button">${iconHtml('fa-camera')} 朋友圈</button>
      <button class="th-wx-tab-add" data-wx-go="newChat" type="button" title="发起会话">${iconHtml('fa-plus')}</button>
    </div>
...
function bubbleInner(m: WxMessage): string {
  ...
  return `<div class="th-wx-text">${esc(m.content)}</div>`;
}
...
function sheetHtml(): string {
  ...
  if (_sheet.kind === 'sticker') {
    ...
    inner = `<div class="th-wx-compose-hint">${esc(hint)}</div>
      <textarea class="th-wx-field th-wx-compose-input" rows="3" placeholder="${isImg ? '一张…的图片' : '（你做了什么 / 场景如何…）'}"></textarea>
...
async function sendCurrentText(): Promise<void> {
  ...
  if (!v) return;
  _pendingUserText = v;
  appendMessage(_view.chatId, { senderId: 'me', kind: 'text', content: v });
  render();
...
function maybeInject(chatId: string): void {
  ...
  const text = msgs.map(m => `${m.senderId === 'me' ? '我' : contactName(m.senderId)}：${m.kind === 'text' ? m.content : m.kind === 'image' ? '[图片]' : m.kind === 'desc' ? '（' + m.content + '）' : '[表情]'}`).join('\n');
...
async function onChatClick(t: HTMLElement): Promise<boolean> {
  ...
  if (row) {
    const isOp = !!t.closest('[data-wx-reroll],[data-wx-edit],[data-wx-recall],[data-wx-delmsg]');
...
function openApp(): void {
  openModal2(`${iconHtml('fa-comment-dots')} 微信`, `<div id="${RID}" class="th-wx"></div>`, {
