...
function loadPresetEnv(): PresetEnvPersist {
  try {
    ...
    if (raw) {
      ...
      return { presetName: p.presetName ?? '', onceOnly: !!p.onceOnly };
    }
  } catch (e) { void e; }
  return { presetName: '', onceOnly: true };
}
...
export type AiMode = 'custom' | 'tavern';
...
// 主组装函数：给 ai-summarize 调。传 aiMode + （可选）覆盖 presetName/onceOnly。
export function resolveGenerateApiConfig(aiMode: AiMode, presetNameOverride?: string, onceOnlyOverride?: boolean): GenerateApiConfig {
  const cfg: GenerateApiConfig = {};
  if (aiMode === 'custom') {
    const p = readApiActivePreset();
    if (p && p.apiurl) {
      const c: CustomApiFromPreset = { source: p.source, apiurl: p.apiurl, model: p.model };
      if (p.key) c.key = p.key;
      // 仅传非 same_as_preset 的自定义值（unset/number）；省略则用预设默认
    ...
    // apiurl 为空（活动预设没填连接）→ 退化用酒馆当前源，不传 custom_api
  }
  // tavern 模式：不传 custom_api，用酒馆当前源
